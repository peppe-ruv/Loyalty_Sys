...
Riferimenti: [Specifica](docs/SPECIFICA.md) · [Parità con Open Loyalty](docs/PARITA-OPEN-LOYALTY.md) · [Catalogo funzionale (edizione attuale)](docs/CATALOGO-FUNZIONALE.md) · [Osservabilità e BI](docs/OSSERVABILITA-BI.md) · [Copertura Loyalty 4.0](docs/COPERTURA-LOYALTY-4.0.md) · [Linee guida UX del backoffice](docs/LINEE-GUIDA-UX-BACKOFFICE.md) · [Decisioni (ADR)](docs/adr/) · [Contratti API](docs/contracts/) · [Runbook](docs/runbooks/) · [Revisione del codice](docs/REVISIONE-CODICE-0.5.0.md) · [CHANGELOG](CHANGELOG.md)
...
| `docs/` | Specifica, ADR, OpenAPI/AsyncAPI, runbook, linee guida UX del backoffice (LG-01..LG-48) |
...
```sh
npm ci            # dipendenze del design system
npm run check     # lint + tipi + test + build
```

I moduli del backoffice importano da qui contratti e regole comuni — workflow D14 (RF-137),
semantica AND/OR delle condizioni (RF-138), frasi senza identificativi tecnici (RF-139), ordine
delle sezioni del form (LG-06), formati italiani di liste e KPI (LG-04, LG-40) — invece di
reimplementarle per modulo: [`web/backoffice-design-system/README.md`](web/backoffice-design-system/README.md).

CI (`ci.yml`): build e test Java, design system (lint, tipi, test, build), build del sito, `helm lint/template`, `terraform validate`, ShellCheck. Release (`release.yml`): su tag `vX.Y.Z` costruisce e firma un'immagine per servizio, genera SBOM, esegue la scansione Trivy e pubblica il chart come OCI.
