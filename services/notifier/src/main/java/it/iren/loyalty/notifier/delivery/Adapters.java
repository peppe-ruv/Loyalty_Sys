...
import it.iren.loyalty.notifier.MessageSender;
        ...
        return new ChannelAdapter() {
            ...
            @Override public Result deliver(Delivery d) {
                ...
                try {
                    ...
                    var res = client.post().uri("").header("X-Loyalty-Signature", new it.iren.loyalty.notifier.webhooks.WebhookSubscription("delivery", "", secret, java.util.Set.of(), Map.of(), true, 0).sign(json.getBytes(java.nio.charset.StandardCharsets.UTF_8))).header("Content-Type", "application/json").body(json).retrieve().toEntity(String.class);
