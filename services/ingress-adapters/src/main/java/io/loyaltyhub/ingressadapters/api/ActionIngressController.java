...
import io.loyaltyhub.common.idempotency.IdempotencyKeys;
...
/**
 * Ingresso REST per le fonti (CRM, portale clienti, app, partner). Contratto: docs/contracts/openapi-ingress.yaml.
...
public class ActionIngressController {
    ...
    private final io.loyaltyhub.ingressadapters.schema.SchemaRegistry schemas;
    private final io.loyaltyhub.ingressadapters.catalog.ProductCatalog catalog;
    private final io.loyaltyhub.common.metrics.LoyaltyMetrics metrics;

    public ActionIngressController(DedupService dedup, ActionPublisher publisher, io.loyaltyhub.ingressadapters.schema.SchemaRegistry schemas, io.loyaltyhub.ingressadapters.catalog.ProductCatalog catalog, io.loyaltyhub.common.metrics.LoyaltyMetrics metrics) {
        this.dedup = dedup;
    ...
    public ResponseEntity<BatchResult> ingest(@RequestBody @Valid IngressBatch batch) {
        ...
        for (IngressItem item : batch.items()) {
            ...
            if (a.attributes() != null && a.attributes().get("lines") != null) {
                var enriched = catalog.enrich(io.loyaltyhub.common.event.TransactionLine.fromAttribute(a.attributes().get("lines")));
            ...
            try {
                publisher.publish(CanonicalEvents.action("urn:loyaltyhub:source:" + source, item.memberId(), a));
