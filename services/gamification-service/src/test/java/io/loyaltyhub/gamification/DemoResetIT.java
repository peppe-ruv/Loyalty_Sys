...
class DemoResetIT {
    ...
    void testDemoResetRestoresState() {
        // 1. Snapshot iniziale
        JsonNode snapshot = client().get().uri("/v1/contests").retrieve().body(JsonNode.class);

        // 2. Mutazione dello stato
        send("POST", "/v1/demo/contests/IW-AUTUNNO/plant-instant", "ADMIN:giuseppe", Map.of("prizeCode", "PTS-100"), 200);




        // 3. Reset demo
        send("POST", "/v1/demo/reset", "ADMIN:giuseppe", null, 200);

        // 4. Verifica ripristino stato
        JsonNode restored = client().get().uri("/v1/contests").retrieve().body(JsonNode.class);
        ...
        for (int i = 0; i < snapshot.size(); i++) {
            ...
            assertThat(s1.path("instants").path("total").asLong()).isEqualTo(s2.path("instants").path("total").asLong());
        }

        // 5. Verifica idempotenza del reset
        send("POST", "/v1/demo/reset", "ADMIN:giuseppe", null, 200);
        JsonNode restored2 = client().get().uri("/v1/contests").retrieve().body(JsonNode.class);
