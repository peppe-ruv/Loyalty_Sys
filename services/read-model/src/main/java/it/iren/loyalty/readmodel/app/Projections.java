package it.iren.loyalty.readmodel.app;
...
import it.iren.loyalty.readmodel.domain.ContextProjector;
...
public class Projections {
    ...
    public void onMovement(byte[] payload) {
        ...
        store.update(CanonicalEvents.memberId(e), c -> {
            var w = c.loyalty().wallets().getOrDefault(wallet, new it.iren.loyalty.readmodel.domain.CustomerContext.Wallet(0, 0, 0, 0, 0, 0));
            ...
            if ("STATUS".equals(wallet) && amount > 0) next = new it.iren.loyalty.readmodel.domain.CustomerContext(next.memberId(), next.identity(),
                    new it.iren.loyalty.readmodel.domain.CustomerContext.Loyalty(next.loyalty().wallets(), next.loyalty().tier(), next.loyalty().statusPointsYear() + amount, next.loyalty().tierSince(), next.loyalty().segments()),
