 ...
 * CUSTOM_DATES, INTERVAL_MONTHS e ANNUAL_ONE_LEVEL (la discesa morbida di D06, default Iren). Benefici per tier: sconto
...
public record TierSet(String id, String name, boolean active, List<Condition> conditions, Match match, List<Tier> tiers, Downgrade downgrade) {
    ...
    /** Il tier set di default Iren (D05/D06): una condizione sui punti STATUS dell'anno, discesa morbida annuale. */
    public static TierSet example() {
        return new TierSet("default", "Programma Iren", true, List.of(new Condition("status", Metric.EARNED_UNITS_IN_PERIOD, "STATUS", 12)), Match.ALL, List.of(
