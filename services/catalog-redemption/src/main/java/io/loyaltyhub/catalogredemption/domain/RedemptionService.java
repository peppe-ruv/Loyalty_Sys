 ...
 * in fattura tramite il sistema di fatturazione, spedizione) è una saga separata avviata dall'evento di riscatto (ADR-010, RewardFulfiller).
...
public class RedemptionService {
    ...
    public RedemptionService(JdbcTemplate jdbc, LedgerPort ledger, CouponPool coupons, io.loyaltyhub.common.metrics.LoyaltyMetrics metrics, org.springframework.kafka.core.KafkaTemplate<String, byte[]> kafka) {
        this.jdbc = jdbc;
        this.ledger = ledger;
    ...
    private void publish(String memberId, UUID redemptionId, String rewardId, String rewardType, String rewardName, String status, long points, String code) {
        ...
        kafka.send(io.loyaltyhub.common.event.EventTypes.TOPIC_REDEMPTIONS, memberId, io.loyaltyhub.common.event.CanonicalEvents.serialize(
                io.loyaltyhub.common.event.CanonicalEvents.of(io.loyaltyhub.common.event.EventTypes.REDEMPTION_V1, "urn:loyaltyhub:catalog", "member:" + memberId, data)));
    }
