...
/** Riga di elenco campagne (docs/servizi/campaign-service.md §3): metadati + totali. */
public record CampaignSummary(
    ...
    public static CampaignSummary of(Campaign c, CounterRepository.Totals totals) {
        return new CampaignSummary(c.id(), c.code(), c.name(), c.status().name(), c.priority(),
                c.triggerActionTypes(), c.visibleInPortal(), c.system(), totals);
    }
}
