package it.iren.loyalty.segmentservice.app;
 ...
 * l'azione interna SEGMENT_ENTERED (D08), idempotente per membro+segmento+giorno.
 */
@Component
public class SegmentRecompute {
    ...
    private void emit(String memberId, String segmentId, boolean entered, Instant now) {
        var membership = CanonicalEvents.of(EventTypes.SEGMENT_MEMBERSHIP_V1, "urn:iren:loyalty:segments", "member:" + memberId,
        ...
        if (entered) {
            ...
            var event = CanonicalEvents.of(EventTypes.ACTION_V1, "urn:iren:loyalty:segments", "member:" + memberId, action);
