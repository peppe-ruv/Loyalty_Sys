 ...
 *
 * <p>Copre i tipi di earning rule di Open Loyalty: general spending ({@link #pointsPerEur}), multiply earned points
 * ({@link #multiplier}, anche per etichetta tramite {@link LineFilter}), product purchase (condizioni su righe),
 * custom event / event rule (tipo azione), referral, geolocation ({@link Operator#GEO_WITHIN}), QR code
 * (azione CODE_REDEEMED), instant reward (campo {@link #autoRewardId}).
 */
public record Rule(
        String id,
        String version,
    ...
    public record LineFilter(Set<String> includeSkus, Set<String> excludeSkus, Set<String> includeLabels, Set<String> excludeLabels, Set<String> excludeCategories) {
        public boolean accepts(it.iren.loyalty.common.event.TransactionLine l) {
