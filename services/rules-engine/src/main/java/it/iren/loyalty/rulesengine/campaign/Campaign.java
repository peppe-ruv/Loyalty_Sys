...
/**
 * Campagna (RF-80..RF-86), modello a parità con le "campaigns" di Open Loyalty: trigger, fino a 6 regole con al più
 * 30 condizioni ciascuna, effetti, limiti e budget, visibilità, attributi custom, unità con scadenza/sospensione
 * calcolate da formula. Versionata e immutabile come le regole (RF-06). {@link it.iren.loyalty.rulesengine.domain.Rule}
...
public record Campaign(
        ...
        List<Rule> rules,
        Limits limits,
        /** ordine di esecuzione fra campagne concorrenti (Open Loyalty: data di inizio); a parità, l'ordine di pubblicazione */
        int displayOrder,
    ...
    /**
     * Condizione: attributo + operatore + valore (forma dichiarativa, come {@link it.iren.loyalty.rulesengine.domain.Rule.Condition})
     * oppure espressione libera sul contesto (RF-84), es. {@code #transaction['amountEur'] > 50 and #member['tier'] == 'TOP'}.
     */
    public record Condition(String attribute, it.iren.loyalty.rulesengine.domain.Rule.Operator op, String value, String expression) {
        public static Condition expr(String e) { return new Condition(null, null, null, e); }
        public static Condition of(String a, it.iren.loyalty.rulesengine.domain.Rule.Operator op, String v) { return new Condition(a, op, v, null); }
    ...
    public static Campaign fromRule(it.iren.loyalty.rulesengine.domain.Rule r) {
