 ...
 * con gli stessi limiti e budget. Il pubblico è paginato dal read-model (tetto 200.000 membri per campagna).
 */
@Component
    ...
    public void tick() {
        ...
        for (Campaign c : campaigns.scheduled()) {
            ...
            audience.membersOf(c).forEach(memberId -> {
                ...
                kafka.send(EventTypes.TOPIC_ACTIONS, memberId, CanonicalEvents.serialize(CanonicalEvents.of(EventTypes.ACTION_V1, "urn:loyaltyhub:automation", "member:" + memberId, a)));
            });
        }
    }
