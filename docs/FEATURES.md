...
| Grafo identità | Identificatori multi-tipo → membro, risoluzione deterministica, collegamenti probabilistici a confidenza ridotta, merge con trasferimento di unità, unmerge da snapshot, alias storici | Multi-type identifiers → member, deterministic resolution, lower-confidence probabilistic links, merge with units transfer, snapshot-based unmerge, historical aliases | RF-136 | `identity-mapping` | ✅ |
...
| Ruoli e permessi | Permessi granulari per collezione e campo, SSO aziendale, chiavi API per fonte di integrazione | Granular per-collection and per-field permissions, corporate SSO, per-source API keys | RF-43, RF-112 | `cms` | ✅ |
