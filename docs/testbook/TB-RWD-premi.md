...
- **AMBIGUO** = la specifica tace e `docs/15` non registra una scelta: la riga asserisce il comportamento di oggi (commento `// TESTBOOK: ambiguo, vedi <ID>` nel test) ed è elencata in §19.
...
| `application/RedemptionService.java:233` | annullo membro senza `memberId` ⇒ ammesso | **ramo senza specifica** | FUL-025 (AMBIGUO) |
...
| `application/RedemptionService.java:334` | premio AUTO_COUPON senza pool ⇒ needsAttention | **ramo senza specifica** | FUL-006 (AMBIGUO) |
...
| `application/CatalogAdminService.java:162-164` | 409 CODE_IMMUTABLE | **ramo senza specifica** | EDT-029 (AMBIGUO) |
...
| `infra/RewardRepository.java:118-124` | ripristino `least(residuo + 1, totale)` | R-09…R-12 | SAG, EDT-053 (AMBIGUO) |
...
| `lh-common …/approval/GovernedTransitions.java:44-46` | approvazione spenta: SUBMIT da DRAFT ⇒ LIVE | **ramo senza specifica** (docs/06 §7: solo «DRAFT → LIVE diretto») | LCY-095 (AMBIGUO) |
...
| `application/CouponService.java:266-271` | emissione: pool vuoto ⇒ vuoto; scadenza = adesso + N × 24 h | R-15, R-17 | FUL-002 (AMBIGUO), EFF |
| `infra/CouponRepository.java:175` | job: ISSUED con `expires_at ≤ asOf` ⇒ EXPIRED | R-15 | CPN-040…050 |
| `api/RewardJobsController.java:62-70` | `asOf` assente = adesso; con `T` istante; data pura = fine giorno Roma | R-15, R-10 (data pura: **ramo senza specifica**) | CPN-044…050, TMO-006/007 |
...
| TB-RWD-VIS-043 | inizio validità = adesso: stato LIVE; validità [0, +1d); tier ammessi tutti, membro GOLD; segmenti tutti, membro in nessuno; membro ACTIVE; stock 100 | **AMBIGUO** — nel catalogo, senza lucchetto, stockState AVAILABLE, perMemberLimitReached false; richiesta 202 PENDING, stock → 99 | reward §3 (catalogo, 422), docs/03 §5, F-RWD-03/04, PT-03/04 | TestbookRwdEligibilityIT#visibility |
...
| TB-RWD-VIS-046 | fine validità = adesso: stato LIVE; validità [-1d, 0); tier ammessi tutti, membro GOLD; segmenti tutti, membro in nessuno; membro ACTIVE; stock 100 | **AMBIGUO** — escluso dal catalogo, dettaglio 404; richiesta 422 REWARD_NOT_AVAILABLE, stock invariato, nessuna richiesta creata | reward §3 (catalogo, 422), docs/03 §5, F-RWD-03/04, PT-03/04 | TestbookRwdEligibilityIT#visibility |
...
| TB-RWD-VIS-058 | limite 1, solo richieste respinte/annullate: stato LIVE; validità [-1d, +1d); tier ammessi tutti, membro GOLD; segmenti tutti, membro in nessuno; membro ACTIVE; stock 100; limite 1, già attive 0, chiuse 2 | **AMBIGUO** — nel catalogo, senza lucchetto, stockState AVAILABLE, perMemberLimitReached false; richiesta 202 PENDING, stock → 99 | reward §3 (catalogo, 422), docs/03 §5, F-RWD-03/04, PT-03/04 | TestbookRwdEligibilityIT#visibility |
...
**Regola**: R-03. La specifica elenca i codici ma **non** la loro precedenza quando più condizioni sono vere; `docs/17` US-E05-05 riporta l'ordine del codice (MEMBER_NOT_ACTIVE → REWARD_NOT_AVAILABLE → TIER_NOT_ELIGIBLE → SHIPPING_REQUIRED → MEMBER_LIMIT_REACHED → REWARD_SOLD_OUT). **Strategia**: una riga per **coppia** di condizioni vere (C(6,2) = 15); ogni riga verifica l'insieme ammesso (specifica) e il codice di oggi (AMBIGUO). Più le richieste malformate (400).

| ID | condizioni/valori | atteso (da spec) | rif. spec | test |
|---|---|---|---|---|
| TB-RWD-ORD-001 | membro BLOCKED + premio PAUSED | **AMBIGUO** — 422 con code ∈ {MEMBER_NOT_ACTIVE, REWARD_NOT_AVAILABLE}; oggi MEMBER_NOT_ACTIVE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-002 | membro BLOCKED + tier non ammesso | **AMBIGUO** — 422 con code ∈ {MEMBER_NOT_ACTIVE, TIER_NOT_ELIGIBLE}; oggi MEMBER_NOT_ACTIVE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-003 | membro BLOCKED + fisico senza spedizione | **AMBIGUO** — 422 con code ∈ {MEMBER_NOT_ACTIVE, SHIPPING_REQUIRED}; oggi MEMBER_NOT_ACTIVE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-004 | membro BLOCKED + limite raggiunto | **AMBIGUO** — 422 con code ∈ {MEMBER_NOT_ACTIVE, MEMBER_LIMIT_REACHED}; oggi MEMBER_NOT_ACTIVE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-005 | membro BLOCKED + esaurito | **AMBIGUO** — 422 con code ∈ {MEMBER_NOT_ACTIVE, REWARD_SOLD_OUT}; oggi MEMBER_NOT_ACTIVE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-006 | premio PAUSED + tier non ammesso | **AMBIGUO** — 422 con code ∈ {REWARD_NOT_AVAILABLE, TIER_NOT_ELIGIBLE}; oggi REWARD_NOT_AVAILABLE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-007 | premio PAUSED + fisico senza spedizione | **AMBIGUO** — 422 con code ∈ {REWARD_NOT_AVAILABLE, SHIPPING_REQUIRED}; oggi REWARD_NOT_AVAILABLE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-008 | premio PAUSED + limite raggiunto | **AMBIGUO** — 422 con code ∈ {REWARD_NOT_AVAILABLE, MEMBER_LIMIT_REACHED}; oggi REWARD_NOT_AVAILABLE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-009 | premio PAUSED + esaurito | **AMBIGUO** — 422 con code ∈ {REWARD_NOT_AVAILABLE, REWARD_SOLD_OUT}; oggi REWARD_NOT_AVAILABLE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-010 | tier non ammesso + fisico senza spedizione | **AMBIGUO** — 422 con code ∈ {TIER_NOT_ELIGIBLE, SHIPPING_REQUIRED}; oggi TIER_NOT_ELIGIBLE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-011 | tier non ammesso + limite raggiunto | **AMBIGUO** — 422 con code ∈ {TIER_NOT_ELIGIBLE, MEMBER_LIMIT_REACHED}; oggi TIER_NOT_ELIGIBLE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-012 | tier non ammesso + esaurito | **AMBIGUO** — 422 con code ∈ {TIER_NOT_ELIGIBLE, REWARD_SOLD_OUT}; oggi TIER_NOT_ELIGIBLE (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-013 | fisico senza spedizione + limite raggiunto | **AMBIGUO** — 422 con code ∈ {SHIPPING_REQUIRED, MEMBER_LIMIT_REACHED}; oggi SHIPPING_REQUIRED (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-014 | fisico senza spedizione + esaurito | **AMBIGUO** — 422 con code ∈ {SHIPPING_REQUIRED, REWARD_SOLD_OUT}; oggi SHIPPING_REQUIRED (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
| TB-RWD-ORD-015 | limite raggiunto + esaurito | **AMBIGUO** — 422 con code ∈ {MEMBER_LIMIT_REACHED, REWARD_SOLD_OUT}; oggi MEMBER_LIMIT_REACHED (primo nell'ordine dei controlli); stock invariato | reward §3 (errori 422), docs/03 §5; ordine in docs/17 US-E05-05 | TestbookRwdEligibilityIT#order |
...
| TB-RWD-STK-012 | stock residuo negativo −1/5 (dato incoerente) | **AMBIGUO** — stockState SOLD_OUT | reward §3 (stockState AVAILABLE/LOW/SOLD_OUT), §3 stats (< 10 %), BO-10 note | TestbookRwdRulesTest#stockState |
...
| TB-RWD-CAT-007 | POST /v1/rewards — tipo minuscolo con spazi « coupon » (type= coupon ;fulfilment=AUTO_COUPON;couponPoolId=<pool>) | **AMBIGUO** — 201, stato DRAFT, version 0, stockRemaining = stockTotal | reward §2, §3; F-RWD-01/03/08; docs/06 §2–3; docs/08 §2 object.edit | TestbookRwdCatalogIT#create |
...
| TB-RWD-CAT-010 | POST /v1/rewards — AUTO_COUPON senza pool (type=COUPON;fulfilment=AUTO_COUPON) | **AMBIGUO** — 201, stato DRAFT, version 0, stockRemaining = stockTotal | reward §2, §3; F-RWD-01/03/08; docs/06 §2–3; docs/08 §2 object.edit | TestbookRwdCatalogIT#create |
...
| TB-RWD-CAT-016 | POST /v1/rewards — categoria assente (category=<null>) | **AMBIGUO** — 201, stato DRAFT, version 0, stockRemaining = stockTotal | reward §2, §3; F-RWD-01/03/08; docs/06 §2–3; docs/08 §2 object.edit | TestbookRwdCatalogIT#create |
...
| TB-RWD-CAT-021 | POST /v1/rewards — perMemberLimit 0 (perMemberLimit=0) | **AMBIGUO** — 422 REWARD_INVALID | reward §2, §3; F-RWD-01/03/08; docs/06 §2–3; docs/08 §2 object.edit | TestbookRwdCatalogIT#create |
...
| TB-RWD-EDT-029 | PUT /v1/rewards/{id} — cambio del codice (code=<other>) | **AMBIGUO** — 409 CODE_IMMUTABLE | reward §3 (modifica ammessa in DRAFT/REJECTED/PAUSED; in LIVE solo stock_total, valid_to, image_url), docs/06 §2, docs/08 §3.2 | TestbookRwdCatalogIT#edit |
...
| TB-RWD-EDT-049 | premio LIVE: PUT senza version (ultima scrittura vince) | **AMBIGUO** — 200; stockTotal 12, stockRemaining 12 | reward §3 (LIVE: stock_total modificabile), §5 (stock prenotato); docs/12 M7.6; Q-112 (409 VERSION_CONFLICT) | TestbookRwdCatalogIT#stockRecalculation |
...
| TB-RWD-EDT-051 | premio LIVE: illimitato con 2 richieste in corso, nuovo totale 10 | **AMBIGUO** — 200; stockTotal 10, stockRemaining 10 | reward §3 (LIVE: stock_total modificabile), §5 (stock prenotato); docs/12 M7.6; Q-112 (409 VERSION_CONFLICT) | TestbookRwdCatalogIT#stockRecalculation |
...
| TB-RWD-EDT-053 | totale 5, 3 prenotate → totale 2 (residuo 0) → una richiesta respinta dal wallet | **AMBIGUO** — residuo 1 (ripristino +1 fino al totale) | docs/03 §5 (REJECTED ripristina lo stock) | TestbookRwdCatalogIT |
...
| TB-RWD-EDT-061 | seconda duplica dello stesso premio | **AMBIGUO** — 201, codice <code>-COPY2 | reward §3, Q-113 | TestbookRwdCatalogIT |
...
| TB-RWD-BND-012 | scala L1 < L2 < L3 (sortOrder crescente): soglia 0 | **AMBIGUO** — 422 BAND_INVALID | reward §3 (soglie uniche e crescenti, 422 BAND_THRESHOLD_DUPLICATE, 409 BAND_IN_USE), F-RWD-02, BO-11, docs/08 §2 | TestbookRwdCatalogIT#bands |
...
| TB-RWD-LCY-095 | approvazione spenta: SUBMIT da DRAFT pubblica | **AMBIGUO** — → LIVE | docs/03 §3.6, docs/06 §3 e §7 (REWARD: LEGAL sempre), docs/08 §2–3.3, F-RWD-08, F-APR-01/02 | TestbookRwdLifecycleTest#transition |
...
| TB-RWD-LCY-098 | azione in minuscolo | **AMBIGUO** — → IN_REVIEW | docs/03 §3.6, docs/06 §3 e §7 (REWARD: LEGAL sempre), docs/08 §2–3.3, F-RWD-08, F-APR-01/02 | TestbookRwdLifecycleTest#transition |
| TB-RWD-LCY-099 | azione sconosciuta | **AMBIGUO** — 422 INVALID_ACTION | docs/03 §3.6, docs/06 §3 e §7 (REWARD: LEGAL sempre), docs/08 §2–3.3, F-RWD-08, F-APR-01/02 | TestbookRwdLifecycleTest#transition |
...
| TB-RWD-FUL-002 | coupon emesso alle 10:00 del 20/10 (Roma CEST) da un pool di validità 10 giorni | **AMBIGUO** — expiresAt = emissione + 10 × 24 h (2026-10-30T10:00Z) | reward §5 («scadenza = oggi + validity_days») | TestbookRwdSagaIT |
...
| TB-RWD-FUL-006 | AUTO_COUPON su premio senza pool → wallet.points.spent | **AMBIGUO** — resta CONFIRMED con needsAttention | nessuna (ramo senza specifica) | TestbookRwdSagaIT |
...
| TB-RWD-FUL-025 | annullo dal portale senza memberId | **AMBIGUO** — 200 CANCELLED | reward §3 portale (identità esplicita) | TestbookRwdSagaIT |
...
| TB-RWD-FUL-028 | POST /v1/portal/redemptions con intestazione X-Correlation-Id: C (come la aggiunge il proxy, docs/07 §3) | **AMBIGUO** — oggi correlationId = id del fatto requested (nuova radice), l'intestazione è ignorata | reward §5 («il correlationId della richiesta HTTP diventa quello di tutta la saga»), docs/07 §3 | TestbookRwdSagaIT |
...
| TB-RWD-ROL-015 | retry-fulfilment, X-LH-Actor ADMIN | **AMBIGUO** — 200 e transizione eseguita | docs/08 §2 redemption.handle (ADMIN, CARE ●), docs/06 §3, reward §3 | TestbookRwdSagaIT#roles |
...
| TB-RWD-ROL-018 | retry-fulfilment, X-LH-Actor CARE | **AMBIGUO** — 200 e transizione eseguita | docs/08 §2 redemption.handle (ADMIN, CARE ●), docs/06 §3, reward §3 | TestbookRwdSagaIT#roles |
...
| TB-RWD-CPN-002 | AVAILABLE (mai emesso) × annulla | **AMBIGUO** — 200; stato dopo VOID | docs/03 §5 (AVAILABLE → ISSUED → USED oppure EXPIRED/VOID), reward §3 (409 COUPON_ALREADY_USED, 410 COUPON_EXPIRED, 404), F-CPN-03, BO-12, PT-13 | TestbookRwdCouponIT#lifecycle |
...
| TB-RWD-CPN-008 | ISSUED oltre la scadenza, job non ancora eseguito × annulla | **AMBIGUO** — 200; stato dopo VOID | docs/03 §5 (AVAILABLE → ISSUED → USED oppure EXPIRED/VOID), reward §3 (409 COUPON_ALREADY_USED, 410 COUPON_EXPIRED, 404), F-CPN-03, BO-12, PT-13 | TestbookRwdCouponIT#lifecycle |
...
| TB-RWD-CPN-011 | EXPIRED (job eseguito) × annulla | **AMBIGUO** — 200; stato dopo VOID | docs/03 §5 (AVAILABLE → ISSUED → USED oppure EXPIRED/VOID), reward §3 (409 COUPON_ALREADY_USED, 410 COUPON_EXPIRED, 404), F-CPN-03, BO-12, PT-13 | TestbookRwdCouponIT#lifecycle |
...
| TB-RWD-CPN-023 | uso all'istante della scadenza | **AMBIGUO** — 410 COUPON_EXPIRED; stato dopo EXPIRED | docs/03 §5 (AVAILABLE → ISSUED → USED oppure EXPIRED/VOID), reward §3 (409 COUPON_ALREADY_USED, 410 COUPON_EXPIRED, 404), F-CPN-03, BO-12, PT-13 | TestbookRwdCouponIT#lifecycle |
...
| TB-RWD-CPN-025 | uso con codice scritto in minuscolo | **AMBIGUO** — 200; stato dopo USED | docs/03 §5 (AVAILABLE → ISSUED → USED oppure EXPIRED/VOID), reward §3 (409 COUPON_ALREADY_USED, 410 COUPON_EXPIRED, 404), F-CPN-03, BO-12, PT-13 | TestbookRwdCouponIT#lifecycle |
...
| TB-RWD-CPN-041 | job expire-coupons: asOf all'istante della scadenza (expiresAt 2026-10-10T10:00:00Z, asOf 2026-10-10T10:00:00Z) | **AMBIGUO** — stato EXPIRED | reward §5 (job: ISSUED scaduti → EXPIRED), §3 demo (?asOf=), docs/08 BO-30, US-E05-17 | TestbookRwdCouponIT#expiry |
...
| TB-RWD-CPN-044 | job expire-coupons: asOf data «2026-10-25» (giorno del cambio d'ora), scadenza 23:59:59 CET (expiresAt 2026-10-25T22:59:59Z, asOf 2026-10-25) | **AMBIGUO** — stato EXPIRED | reward §5 (job: ISSUED scaduti → EXPIRED), §3 demo (?asOf=), docs/08 BO-30, US-E05-17 | TestbookRwdCouponIT#expiry |
| TB-RWD-CPN-045 | job expire-coupons: asOf data «2026-10-25», scadenza 26/10 00:00 CET (expiresAt 2026-10-25T23:00:00Z, asOf 2026-10-25) | **AMBIGUO** — stato EXPIRED | reward §5 (job: ISSUED scaduti → EXPIRED), §3 demo (?asOf=), docs/08 BO-30, US-E05-17 | TestbookRwdCouponIT#expiry |
| TB-RWD-CPN-046 | job expire-coupons: asOf data «2026-03-29» (cambio d'ora di marzo), scadenza 23:59:59 CEST (expiresAt 2026-03-29T21:59:59Z, asOf 2026-03-29) | **AMBIGUO** — stato EXPIRED | reward §5 (job: ISSUED scaduti → EXPIRED), §3 demo (?asOf=), docs/08 BO-30, US-E05-17 | TestbookRwdCouponIT#expiry |
| TB-RWD-CPN-047 | job expire-coupons: asOf data «2026-03-29», scadenza 30/3 00:00 CEST (expiresAt 2026-03-29T22:00:00Z, asOf 2026-03-29) | **AMBIGUO** — stato EXPIRED | reward §5 (job: ISSUED scaduti → EXPIRED), §3 demo (?asOf=), docs/08 BO-30, US-E05-17 | TestbookRwdCouponIT#expiry |
| TB-RWD-CPN-048 | job expire-coupons: asOf data «2028-02-29» (bisestile), scadenza a metà giornata (expiresAt 2028-02-29T12:00:00Z, asOf 2028-02-29) | **AMBIGUO** — stato EXPIRED | reward §5 (job: ISSUED scaduti → EXPIRED), §3 demo (?asOf=), docs/08 BO-30, US-E05-17 | TestbookRwdCouponIT#expiry |
...
| TB-RWD-CPN-061 | POST /v1/coupon-pools — prefisso e codice in minuscolo | **AMBIGUO** — 201, conteggi per stato a zero | F-CPN-01, reward §2 (coupon_pool), §3, docs/08 §2 object.edit (pool coupon) | TestbookRwdCouponIT#createPool |
| TB-RWD-CPN-062 | POST /v1/coupon-pools — prefisso «caf!» | **AMBIGUO** — 422 COUPON_PREFIX_INVALID | F-CPN-01, reward §2 (coupon_pool), §3, docs/08 §2 object.edit (pool coupon) | TestbookRwdCouponIT#createPool |
| TB-RWD-CPN-063 | POST /v1/coupon-pools — prefisso di 1 carattere | **AMBIGUO** — 422 COUPON_PREFIX_INVALID | F-CPN-01, reward §2 (coupon_pool), §3, docs/08 §2 object.edit (pool coupon) | TestbookRwdCouponIT#createPool |
| TB-RWD-CPN-064 | POST /v1/coupon-pools — prefisso di 2 caratteri | **AMBIGUO** — 201, conteggi per stato a zero | F-CPN-01, reward §2 (coupon_pool), §3, docs/08 §2 object.edit (pool coupon) | TestbookRwdCouponIT#createPool |
| TB-RWD-CPN-065 | POST /v1/coupon-pools — prefisso di 10 caratteri | **AMBIGUO** — 201, conteggi per stato a zero | F-CPN-01, reward §2 (coupon_pool), §3, docs/08 §2 object.edit (pool coupon) | TestbookRwdCouponIT#createPool |
| TB-RWD-CPN-066 | POST /v1/coupon-pools — prefisso di 11 caratteri | **AMBIGUO** — 422 COUPON_PREFIX_INVALID | F-CPN-01, reward §2 (coupon_pool), §3, docs/08 §2 object.edit (pool coupon) | TestbookRwdCouponIT#createPool |
| TB-RWD-CPN-067 | POST /v1/coupon-pools — validità 0 giorni | **AMBIGUO** — 422 COUPON_VALIDITY_INVALID | F-CPN-01, reward §2 (coupon_pool), §3, docs/08 §2 object.edit (pool coupon) | TestbookRwdCouponIT#createPool |
...
| TB-RWD-CPN-069 | POST /v1/coupon-pools — validità 3650 giorni | **AMBIGUO** — 201, conteggi per stato a zero | F-CPN-01, reward §2 (coupon_pool), §3, docs/08 §2 object.edit (pool coupon) | TestbookRwdCouponIT#createPool |
| TB-RWD-CPN-070 | POST /v1/coupon-pools — validità 3651 giorni | **AMBIGUO** — 422 COUPON_VALIDITY_INVALID | F-CPN-01, reward §2 (coupon_pool), §3, docs/08 §2 object.edit (pool coupon) | TestbookRwdCouponIT#createPool |
| TB-RWD-CPN-071 | POST /v1/coupon-pools — validità assente | **AMBIGUO** — 201, conteggi per stato a zero | F-CPN-01, reward §2 (coupon_pool), §3, docs/08 §2 object.edit (pool coupon) | TestbookRwdCouponIT#createPool |
...
| TB-RWD-CPN-083 | POST /v1/coupon-pools/{id}/generate — genera 0 codici | **AMBIGUO** — 422 COUPON_COUNT_INVALID; codici AVAILABLE nel pool: 0 | reward §3 ({count ≤ 5000} → prefix-XXXX-XXXX), docs/03 §5 (A-Z2-9), reward §6 e docs/10 §5 (seme fisso), F-CPN-01 | TestbookRwdCouponIT#generate |
| TB-RWD-CPN-084 | POST /v1/coupon-pools/{id}/generate — genera −1 codici | **AMBIGUO** — 422 COUPON_COUNT_INVALID; codici AVAILABLE nel pool: 0 | reward §3 ({count ≤ 5000} → prefix-XXXX-XXXX), docs/03 §5 (A-Z2-9), reward §6 e docs/10 §5 (seme fisso), F-CPN-01 | TestbookRwdCouponIT#generate |
...
| TB-RWD-CPN-095 | POST /v1/coupon-pools/{id}/import — codice troppo corto «ab» | **AMBIGUO** — 200; imported 1, skipped 1; importati visibili come AVAILABLE | reward §3 (import {codes[]}; duplicati → {imported, skipped[]}), F-CPN-01, BO-12 | TestbookRwdCouponIT#importCodes |
| TB-RWD-CPN-096 | POST /v1/coupon-pools/{id}/import — codice minuscolo con spazi «  abcd-efgh » | **AMBIGUO** — 200; imported 1, skipped 0; importati visibili come AVAILABLE | reward §3 (import {codes[]}; duplicati → {imported, skipped[]}), F-CPN-01, BO-12 | TestbookRwdCouponIT#importCodes |
| TB-RWD-CPN-097 | POST /v1/coupon-pools/{id}/import — elenco vuoto | **AMBIGUO** — 422 | reward §3 (import {codes[]}; duplicati → {imported, skipped[]}), F-CPN-01, BO-12 | TestbookRwdCouponIT#importCodes |
| TB-RWD-CPN-098 | POST /v1/coupon-pools/{id}/import — 5001 codici | **AMBIGUO** — 422 | reward §3 (import {codes[]}; duplicati → {imported, skipped[]}), F-CPN-01, BO-12 | TestbookRwdCouponIT#importCodes |
...
| TB-RWD-EFF-005 | premio inesistente | **AMBIGUO** — voce DLQ con lh-error-code REWARD_NOT_FOUND | nessuna (ramo senza specifica) | TestbookRwdCouponIT |
| TB-RWD-EFF-006 | premio senza pool | **AMBIGUO** — voce DLQ con lh-error-code COUPON_POOL_MISSING | nessuna (ramo senza specifica) | TestbookRwdCouponIT |
| TB-RWD-EFF-007 | effetto senza subject member: | **AMBIGUO** — voce DLQ con lh-error-code INVALID_EFFECT | nessuna (ramo senza specifica) | TestbookRwdCouponIT |
| TB-RWD-EFF-008 | effectId assente | **AMBIGUO** — coupon emesso con effectId = id dell'evento | nessuna (ramo senza specifica) | TestbookRwdCouponIT |
...
Righe AMBIGUO (asseriscono il comportamento di oggi, da decidere in `docs/15`):

- **TB-RWD-VIS-043** — estremo iniziale: la specifica dice solo «dentro validità»; oggi incluso
- **TB-RWD-VIS-046** — estremo finale: oggi escluso (validità semiaperta)
- **TB-RWD-VIS-058** — le richieste REJECTED/CANCELLED non contano per il limite (ripristinano lo stock, docs/03 §5)
- **TB-RWD-ORD-001** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-002** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-003** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-004** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-005** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-006** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-007** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-008** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-009** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-010** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-011** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-012** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-013** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-014** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-ORD-015** — la specifica elenca i codici ma non la precedenza quando più condizioni sono vere; il test verifica l'insieme ammesso (specifica) e il codice di oggi
- **TB-RWD-STK-012** — dato incoerente non previsto dalla specifica
- **TB-RWD-CAT-007** — normalizzazione del tipo non prevista dalla specifica
- **TB-RWD-CAT-010** — BO-10 parla di «AUTO con pool coupon» ma nessuna fonte vieta il pool assente
- **TB-RWD-CAT-016** — la categoria facoltativa non è prevista (reward §2 elenca category_code senza null)
- **TB-RWD-CAT-021** — limite 0 non trattato dalla specifica
- **TB-RWD-EDT-029** — l'immutabilità del codice non è scritta (reward §2: code UQ)
- **TB-RWD-EDT-049** — il PUT senza version non è trattato da Q-112
- **TB-RWD-EDT-051** — passaggio da illimitato a limitato: le richieste in corso non sono sottratte
- **TB-RWD-EDT-053** — la specifica dice «ripristina lo stock» ma non come comporlo con un totale ridotto sotto le prenotazioni
- **TB-RWD-EDT-061** — la specifica fissa solo «-COPY»; la collisione non è trattata
- **TB-RWD-BND-012** — soglia nulla (premio gratuito) non trattata dalla specifica
- **TB-RWD-LCY-095** — SUBMIT che pubblica direttamente con approvazione spenta è una scelta di lh-common (docs/06 §7 dice solo «DRAFT → LIVE diretto»)
- **TB-RWD-LCY-098** — maiuscole/minuscole dell'azione non trattate
- **TB-RWD-LCY-099** — azione fuori enumerato: 422 (non 400) non fissato dalla specifica
- **TB-RWD-FUL-002** — «oggi + validity_days» non dice se si contano giorni di calendario di Roma (qui il cambio dell'ora sposterebbe l'ora locale) o istanti; oggi + N × 24 h
- **TB-RWD-FUL-006** — premio AUTO_COUPON senza pool: nessuna fonte ne descrive l'evasione
- **TB-RWD-FUL-025** — annullo senza memberId: l'identità esplicita (CLAUDE.md §1.6) non dice cosa fare se manca
- **TB-RWD-FUL-028** — «correlationId della richiesta HTTP»: l'intestazione X-Correlation-Id del proxy o il correlationId restituito dal 202? Se è l'intestazione, la regola non è implementata
- **TB-RWD-ROL-015** — retry-fulfilment non è elencato in docs/08 §2: si applica redemption.handle
- **TB-RWD-ROL-018** — retry-fulfilment non è elencato in docs/08 §2: si applica redemption.handle
- **TB-RWD-CPN-002** — docs/03 §5 dà VOID come esito del ciclo senza dire da quali stati: annullare un codice mai emesso o già scaduto non è trattato
- **TB-RWD-CPN-008** — docs/03 §5 dà VOID come esito del ciclo senza dire da quali stati: annullare un codice mai emesso o già scaduto non è trattato
- **TB-RWD-CPN-011** — docs/03 §5 dà VOID come esito del ciclo senza dire da quali stati: annullare un codice mai emesso o già scaduto non è trattato
- **TB-RWD-CPN-023** — istante della scadenza: la specifica non dice se è ancora valido (docs/03 §4.2 per i lotti usa expiresAt ≤ asOf ⇒ scaduto)
- **TB-RWD-CPN-025** — normalizzazione del codice alla cassa non prevista
- **TB-RWD-CPN-041** — istante della scadenza: come docs/03 §4.2 (expiresAt ≤ asOf) per i lotti
- **TB-RWD-CPN-044** — asOf come data pura = fine del giorno a Roma: convenzione di BO-30 non scritta
- **TB-RWD-CPN-045** — asOf come data pura: il codice la intende fine del giorno a Roma (23:59:59.999999999), ma Postgres arrotonda al microsecondo e la fa coincidere con la mezzanotte successiva, così scade anche il coupon che scade alle 00:00 del giorno dopo; oggi EXPIRED
- **TB-RWD-CPN-046** — asOf come data pura = fine del giorno a Roma
- **TB-RWD-CPN-047** — come TB-RWD-CPN-045 (fine giornata arrotondata alla mezzanotte successiva); oggi EXPIRED
- **TB-RWD-CPN-048** — asOf come data pura = fine del giorno a Roma
- **TB-RWD-CPN-061** — normalizzazione in maiuscolo non prevista
- **TB-RWD-CPN-062** — regole del prefisso (2–10 A-Z0-9) non scritte
- **TB-RWD-CPN-063** — regole del prefisso non scritte
- **TB-RWD-CPN-064** — regole del prefisso non scritte
- **TB-RWD-CPN-065** — regole del prefisso non scritte
- **TB-RWD-CPN-066** — regole del prefisso non scritte
- **TB-RWD-CPN-067** — limiti di validity_days non scritti
- **TB-RWD-CPN-069** — limiti di validity_days non scritti
- **TB-RWD-CPN-070** — limiti di validity_days non scritti
- **TB-RWD-CPN-071** — valore predefinito 90 giorni non scritto
- **TB-RWD-CPN-083** — codice d'errore di count fuori intervallo non scritto (la specifica fissa solo ≤ 5000)
- **TB-RWD-CPN-084** — codice d'errore di count fuori intervallo non scritto (la specifica fissa solo ≤ 5000)
- **TB-RWD-CPN-095** — validità del formato dei codici importati non scritta
- **TB-RWD-CPN-096** — normalizzazione dei codici importati non scritta
- **TB-RWD-CPN-097** — codice d'errore COUPON_IMPORT_EMPTY non scritto
- **TB-RWD-CPN-098** — il tetto di 5000 è scritto solo per generate
- **TB-RWD-EFF-005** — premio sconosciuto nell'effetto non trattato
- **TB-RWD-EFF-006** — premio senza pool nell'effetto non trattato
- **TB-RWD-EFF-007** — effetto senza membro non trattato
- **TB-RWD-EFF-008** — effectId assente: idempotenza ricadrebbe sull'id dell'evento

Conflitti tra fonti (da registrare in `docs/15`, regola di precedenza applicata):
- **Campi modificabili in LIVE**: docs/03 §3.6 (nome, descrizione, fine, priorità, immagine) vs reward §3 (`stock_total`, `valid_to`, `image_url`). Applicata reward §3 (fonte n. 3): nome e descrizione bloccati (EDT-007, EDT-015).
- **Stato REJECTED del premio**: reward §3 ammette la modifica in `REJECTED`, stato che la macchina comune (docs/03 §3.6) non ha più (REJECT ⇒ DRAFT). Righe scritte sul ritorno in DRAFT (LCY-003, LCY-123).
- **`?status=` di `GET /v1/approvals`**: docs/06 §7 lo cita, l'endpoint lo ignora e restituisce sempre la coda IN_REVIEW (LCY-122 usa il parametro, esito conforme).
...
| Rami senza specifica | 20 voci di §2 (marcate **ramo senza specifica**; le righe che li provano sono AMBIGUO) |
| Regole non implementate | 1 possibile: R-27 «il correlationId della richiesta HTTP diventa quello della saga» se si intende l'intestazione `X-Correlation-Id` del proxy (TB-RWD-FUL-028, AMBIGUO) |
| Righe AMBIGUO | 69 (§19) |
| Divergenze aperte | 3 (§18: TB-RWD-ORD-019, TB-RWD-FUL-011, TB-RWD-FUL-013 — stessa causa in lh-common) |
