...
- **Fonti (oracolo).** docs/04 §5 (affidabilità), docs/05 e `contracts/events/` (envelope, tipi, chiavi, topic),
  ...
  Dove la specifica tace valgono le scelte di docs/15 (Q-131, Q-261, Q-267, Q-298, Q-317, Q-323 e le nuove Q-P1…Q-P11).
...
- **Esito.** 563 righe in 28 aree; 22 cause di divergenza trovate: 21 corrette nel codice (registro in §25), una
  aperta perché richiede una dipendenza nuova (OpenAPI, Q-P10). Nessuna riga AMBIGUO: ogni silenzio della specifica è
...
| R15 | Pulizie: outbox pubblicata > 24 h, `processed_event` > 14 giorni | docs/06 §4; RNF-07; Q-P4 | `OutboxCleanup`, `ProcessedEventCleanup` | REL |
...
| R22 | Elenchi `{items, page}`, `size` massimo 100 | docs/06 §2; Q-P1 | `PageResponse`, `PageParams`, 7 servizi | PAG |
...
| R27 | Espressioni di data relative dei seed in Europe/Rome | docs/10 §1.2; Q-267; Q-P5; Q-P6 | `SeedDates` | CLK |
| R28 | Calendario di business Europe/Rome, chiavi di periodo, edizione | docs/06 §1; docs/10 §1.2 | `BusinessCalendar` | CAL |
| R29 | Variabili d'ambiente della matrice di docs/11 §8 | docs/11 §8; Q-131; Q-P9 | `LhEnvironmentAliases` | VAR |
...
| R34 | Limite di 60 eventi al minuto per IP in ingestion | docs/11 §11; Q-P8 | ingestion `IngressRateLimitFilter` | RLM |
...
| R36 | OpenAPI su `/v3/api-docs` | docs/06 §2 | — (**regola non implementata**, Q-P10) | HLR |
...
| `GlobalExceptionHandler` `LhException` con/senza `errors` · validazione · corpo illeggibile · parametro non convertibile · parametro assente · percorso non mappato · dipendenza giù · errore 4xx di Spring (404/altro) · SSE chiuso · imprevisto | R21 | ERR, HER | coperti; 500 `internal` e 410 `gone` senza specifica (decisi in Q-P2); SSE chiuso senza corpo, senza specifica, coperto in TB-INS |
...
| `DemoResetController#reset` con/senza audit · serializzato; `#info` ambiente/JDBC assenti · versione da proprietà/jar/«sviluppo» | R24–R26 | RST, INF | coperti; «sviluppo» senza specifica (Q-P7) |
| `SeedDates#resolve` vuota · non `@` · suffisso ora · 9 parole chiave · `last<Giorno>` · scostamenti d/h/M/y · fine mese con M/y · testo estraneo | R27 | CLK | coperti; `last<qualunque giorno>` senza specifica (Q-P6) |
...
**Regola.** Si pubblica solo dall'outbox scritta nella transazione del cambiamento di stato (RNF-05, ADR-008); il relay (500 ms, `FOR UPDATE SKIP LOCKED`, lotti da 100) invia su Kafka con chiave e header `lh-type`, `lh-correlation-id` (più causa, hop, attore) e `content-type: application/cloudevents+json`, poi marca `published_at`; con Kafka irraggiungibile la riga resta e parte al ritorno (docs/12 M0); l'ordine per chiave è quello di scrittura (RNF-04); le righe pubblicate da oltre 24 h e i `processed_event` oltre 14 giorni si cancellano (docs/06 §4, RNF-07; Q-P4); `lh_outbox_pending` e `lh_events_published_total` (docs/06 §8).
...
**Regola.** 3 tentativi (1 s, 5 s: Q-131) per gli errori ritentabili, poi `lh.dlq.v1`; gli errori non ritentabili — validazione, deserializzazione (docs/04 §5), `NonRetryableEventException`, `LOOP_GUARD` (docs/05 §7) — al primo tentativo. Header `lh-original-topic`, `lh-consumer`, `lh-error-class`, `lh-error-message`, `lh-attempts` (docs/04 §5) più `lh-error-code`; `lh-attempts` = tentativi fatti con i ritardi configurati. Q-P3: la catena delle cause decide ritentabilità e codice.
...
| TB-PLT-DLR-019 | causa non ritentabile dentro un'altra eccezione: non ritentabile (Q-P3) (`causeNonRetryable` · retryable) | false — divergenza corretta D-06 | Q-P3 | `TestbookPltDlqTest#rules` · `dlq-rules.csv` |
| TB-PLT-DLR-020 | causa non ritentabile dentro un'altra eccezione: codice della causa (Q-P3) (`causeNonRetryable` · code) | TEMPLATE_NOT_FOUND — divergenza corretta D-06 | Q-P3 | `TestbookPltDlqTest#rules` · `dlq-rules.csv` |
| TB-PLT-DLR-021 | causa LOOP_GUARD dentro un'altra eccezione: LOOP_GUARD (Q-P3) (`causeLoopGuard` · code) | LOOP_GUARD — divergenza corretta D-06 | Q-P3 | `TestbookPltDlqTest#rules` · `dlq-rules.csv` |
| TB-PLT-DLR-022 | causa JSON illeggibile dentro un'altra eccezione: non ritentabile (Q-P3) (`causeJson` · retryable) | false — divergenza corretta D-06 | Q-P3 | `TestbookPltDlqTest#rules` · `dlq-rules.csv` |
...
| TB-PLT-DLK-006 | non ritentabile avvolto in un'altra eccezione: subito in DLQ col codice della causa (Q-P3) (`wrapped`, guasti 99, ritardi 50 50) | invocazioni=1 dlq=si tentativi=1 codice=TEMPLATE_NOT_FOUND ritentabile=false topic=origine consumer=gruppo chiave=originale valore=poison lh-type=io.loyaltyhub.fact.probe successivo=elaborato — divergenza corretta D-06 | Q-P3 | `TestbookPltRelayIT#dlq` · `dlq-kafka.csv` |
...
| TB-PLT-ERR-007 | 410 risorsa non più utilizzabile: gone (titolo in italiano; Q-P2) (GET /v1/probe/lh/gone) | 410 gone COUPON_EXPIRED — divergenza corretta D-03 | titolo in italiano; Q-P2 | `TestbookPltErrorsTest#problem` · `errors.csv` |
...
| TB-PLT-ERR-018 | metodo non ammesso: 405 problem bad-request (Q-P2) (POST /v1/probe/only-get) | 405 bad-request BAD_REQUEST — divergenza corretta D-02 | Q-P2 | `TestbookPltErrorsTest#problem` · `errors.csv` |
| TB-PLT-ERR-019 | tipo di contenuto non supportato: 415 problem bad-request (Q-P2) (POST /v1/probe/body) | 415 bad-request BAD_REQUEST — divergenza corretta D-02 | Q-P2 | `TestbookPltErrorsTest#problem` · `errors.csv` |
...
| TB-PLT-HER-009 | metodo non ammesso: 405 bad-request (Q-P2) (DELETE /v1/members) | 405 bad-request BAD_REQUEST — divergenza corretta D-02 | Q-P2 | `TestbookPltApiIT#errors` · `errors-hub.csv` |
| TB-PLT-HER-010 | tipo di contenuto non supportato: 415 bad-request (Q-P2) (POST /v1/members) | 415 bad-request BAD_REQUEST — divergenza corretta D-02 | Q-P2 | `TestbookPltApiIT#errors` · `errors-hub.csv` |


## 16. PAG — Paginazione `{items, page}`

**Regola.** Elenchi con `?page=0&size=20`, risposta `{items, page{number, size, totalItems, totalPages}}`, `size` massimo 100 (docs/06 §2); Q-P1: `size` < 1 o `page` < 0 ⇒ 400 `BAD_REQUEST` in ogni servizio (estende Q-317/Q-323), `size` > 100 ⇒ 100; `totalPages` = ⌈totalItems/size⌉.
...
| ID | condizioni/valori | atteso (da spec) | rif. spec | test |
|---|---|---|---|---|
| TB-PLT-PAG-001 | GET /v1/members size=1 | 200 size=1 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-002 | GET /v1/members size=100 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-003 | GET /v1/members size=101 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-004 | GET /v1/members size=0 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-005 | GET /v1/members page=-1 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-006 | GET /v1/members (senza parametri) | 200 default | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-007 | GET /v1/segments size=1 | 200 size=1 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-008 | GET /v1/segments size=100 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-009 | GET /v1/segments size=101 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-010 | GET /v1/segments size=0 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-011 | GET /v1/segments page=-1 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-012 | GET /v1/segments (senza parametri) | 200 default | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-013 | GET /v1/segments/SEG-DIGITAL/members size=1 | 200 size=1 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-014 | GET /v1/segments/SEG-DIGITAL/members size=100 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-015 | GET /v1/segments/SEG-DIGITAL/members size=101 | 200 size=100 — divergenza corretta D-14 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-016 | GET /v1/segments/SEG-DIGITAL/members size=0 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-017 | GET /v1/segments/SEG-DIGITAL/members page=-1 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-018 | GET /v1/segments/SEG-DIGITAL/members (senza parametri) | 200 default | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-019 | GET /v1/redemptions size=1 | 200 size=1 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-020 | GET /v1/redemptions size=100 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-021 | GET /v1/redemptions size=101 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-022 | GET /v1/redemptions size=0 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-023 | GET /v1/redemptions page=-1 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-024 | GET /v1/redemptions (senza parametri) | 200 default | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-025 | GET /v1/coupon-pools/POOL-CAF/coupons size=1 | 200 size=1 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-026 | GET /v1/coupon-pools/POOL-CAF/coupons size=100 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-027 | GET /v1/coupon-pools/POOL-CAF/coupons size=101 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-028 | GET /v1/coupon-pools/POOL-CAF/coupons size=0 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-029 | GET /v1/coupon-pools/POOL-CAF/coupons page=-1 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-030 | GET /v1/coupon-pools/POOL-CAF/coupons (senza parametri) | 200 default | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-031 | GET /v1/messages size=1 | 200 size=1 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-032 | GET /v1/messages size=100 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-033 | GET /v1/messages size=101 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-034 | GET /v1/messages size=0 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-035 | GET /v1/messages page=-1 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-036 | GET /v1/messages (senza parametri) | 200 default | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-037 | GET /v1/portal/inbox?memberId=MBR-000002 size=1 | 200 size=1 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-038 | GET /v1/portal/inbox?memberId=MBR-000002 size=100 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-039 | GET /v1/portal/inbox?memberId=MBR-000002 size=101 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-040 | GET /v1/portal/inbox?memberId=MBR-000002 size=0 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-041 | GET /v1/portal/inbox?memberId=MBR-000002 page=-1 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-042 | GET /v1/portal/inbox?memberId=MBR-000002 (senza parametri) | 200 default | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-043 | GET /v1/webhooks/WH-CRM-DEMO/deliveries size=1 | 200 size=1 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-044 | GET /v1/webhooks/WH-CRM-DEMO/deliveries size=100 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-045 | GET /v1/webhooks/WH-CRM-DEMO/deliveries size=101 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-046 | GET /v1/webhooks/WH-CRM-DEMO/deliveries size=0 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-047 | GET /v1/webhooks/WH-CRM-DEMO/deliveries page=-1 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-048 | GET /v1/webhooks/WH-CRM-DEMO/deliveries (senza parametri) | 200 default | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-049 | GET /v1/contests/IW-AUTUNNO/instants size=1 | 200 size=1 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-050 | GET /v1/contests/IW-AUTUNNO/instants size=100 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-051 | GET /v1/contests/IW-AUTUNNO/instants size=101 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-052 | GET /v1/contests/IW-AUTUNNO/instants size=0 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-053 | GET /v1/contests/IW-AUTUNNO/instants page=-1 | 400 BAD_REQUEST — divergenza corretta D-14 | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-054 | GET /v1/contests/IW-AUTUNNO/instants (senza parametri) | 200 default | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-055 | GET /v1/events size=1 | 200 size=1 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-056 | GET /v1/events size=100 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-057 | GET /v1/events size=101 | 200 size=100 | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-058 | GET /v1/events size=0 | 400 BAD_REQUEST | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-059 | GET /v1/events page=-1 | 400 BAD_REQUEST | Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
| TB-PLT-PAG-060 | GET /v1/events (senza parametri) | 200 default | docs/06 §2; Q-P1 | `TestbookPltApiIT#pagination` · `pagination.csv` |
...
**Regola.** `POST /v1/demo/reset` (ADMIN) tronca le tabelle del servizio e ricarica i seed, idempotente: due reset danno lo stesso stato, stessi codici coupon e istanti (docs/06 §10, docs/10 §1.3, ADR-015); voce di audit `RESET` con l'attore; insight per primo (BO-30); Q-P7: reset concorrenti serializzati. `GET /v1/demo/info`: profili, versione, conteggi, ultimo reset (docs/06 §10, BO-30). Oracolo di stato: impronta del seed (righe per tabella degli 8 schemi e proiezioni con chiavi naturali).
...
| TB-PLT-RST-007 | due reset concorrenti: entrambi 200 e stato del seed (Q-P7) (`concurrent`) | stato uguale all'impronta del seed — divergenza corretta D-16 | Q-P7 | `TestbookPltApiIT#reset` · `reset.csv` |
...
| TB-PLT-HLR-008 | OpenAPI su /v3/api-docs (docs/06 §2) (`openapi`) | 200 — **DIVERGENZA aperta D-22** | docs/06 §2 | `TestbookPltApiIT#health` · `health.csv` |
...
**Regola.** Ogni servizio legge le variabili della matrice di docs/11 §8 (`LH_CONSUMER_RETRY_BACKOFF_MS`, `LH_JOBS_ENABLED`, `LH_APPROVAL_ENABLED`, `KAFKA_*`, `DB_USER`, `DB_URL_DIRECT`) con la precedenza dell'ambiente; il nome alternativo già in uso (`DB_USERNAME`, `SPRING_KAFKA_BOOTSTRAP_SERVERS` di ADR-025) prevale; default comuni di salute (liveness senza dipendenze, readiness con db e kafka) solo con le sonde accese (Q-P9).
...
| TB-PLT-VAR-002 | `-` → `bind:retryBackoffMs` | [1000, 5000] | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-003 | `LH_CONSUMER_RETRY_BACKOFF_MS=200,300` con `loyaltyhub.consumer.retry-backoff-ms=50,50` → `bind:retryBackoffMs` | [200, 300] — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-004 | `LH_JOBS_ENABLED=true` → `loyaltyhub.jobs.enabled` | true — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-005 | `LH_JOBS_ENABLED=true` con `loyaltyhub.jobs.enabled=false` → `loyaltyhub.jobs.enabled` | true — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-006 | `-` con `loyaltyhub.jobs.enabled=false` → `loyaltyhub.jobs.enabled` | false | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-007 | `LH_JOBS_ENABLED=` con `loyaltyhub.jobs.enabled=false` → `loyaltyhub.jobs.enabled` | false | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-008 | `LH_APPROVAL_ENABLED=false` → `loyaltyhub.approval.enabled` | false — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-009 | `KAFKA_SECURITY=SSL_PEM` → `bind:security` | SSL_PEM — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-010 | `KAFKA_SSL_CA_B64=Q0E=` → `loyaltyhub.kafka.ssl.ca-b64` | Q0E= — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-011 | `KAFKA_SSL_CERT_B64=Q0VSVA==` → `loyaltyhub.kafka.ssl.cert-b64` | Q0VSVA== — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-012 | `KAFKA_SSL_KEY_B64=S0VZ` → `loyaltyhub.kafka.ssl.key-b64` | S0VZ — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-013 | `KAFKA_SASL_USERNAME=lh-user` → `loyaltyhub.kafka.sasl.username` | lh-user — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-014 | `KAFKA_SASL_PASSWORD=s3gr3t0` → `loyaltyhub.kafka.sasl.password` | s3gr3t0 — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
| TB-PLT-VAR-015 | `KAFKA_SASL_MECHANISM=PLAIN` → `loyaltyhub.kafka.sasl.mechanism` | PLAIN — divergenza corretta D-11 | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
...
| TB-PLT-VAR-019 | `DB_USER=lh_app;DB_USERNAME=lh_owner` con `spring.datasource.username=lh_owner` → `spring.datasource.username` | lh_owner | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
...
| TB-PLT-VAR-021 | `-` → `spring.flyway.url` | null | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
...
| TB-PLT-VAR-023 | `-` con `management.endpoint.health.probes.enabled=true;management.endpoint.health.group.readiness.include=readinessState` → `management.endpoint.health.group.readiness.include` | readinessState | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
...
| TB-PLT-VAR-025 | `-` con `management.endpoint.health.probes.enabled=false` → `management.endpoint.health.group.readiness.include` | null | docs/11 §8; Q-P9 | `TestbookPltConfigTest#environment` · `env.csv` |
...
| TB-PLT-CLK-011 | `@eom+1M` alle 2026-09-24T10:00:00Z — @eom+1M da settembre (30 gg): fine di ottobre (31 gg) (Q-P5) | 2026-10-31T22:59:59Z — divergenza corretta D-10 | Q-P5 | `TestbookPltClockTest#seedDates` · `seed-dates.csv` |
| TB-PLT-CLK-012 | `@eom+1M` alle 2027-02-10T10:00:00Z — @eom+1M da febbraio 2027: fine di marzo (Q-P5) | 2027-03-31T21:59:59Z — divergenza corretta D-10 | Q-P5 | `TestbookPltClockTest#seedDates` · `seed-dates.csv` |
...
| TB-PLT-CLK-028 | `@lastSunday` alle 2026-09-24T10:00:00Z — @lastSunday (giorno non in docs/10: esteso a ogni giorno; Q-P6) | 2026-09-19T22:00:00Z | giorno non in docs/10: esteso a ogni giorno; Q-P6 | `TestbookPltClockTest#seedDates` · `seed-dates.csv` |
...
| TB-PLT-CLK-059 | `@today-1dx+2d` alle 2026-09-24T10:00:00Z — testo estraneo tra due scostamenti: non valido (Q-P5) | ERR:IllegalArgumentException — divergenza corretta D-10 | Q-P5 | `TestbookPltClockTest#seedDates` · `seed-dates.csv` |
...
**Regola.** Latenza azione → movimento p50 ≤ 3 s, p95 ≤ 8 s a servizi svegli (RNF-02); ordine per membro (RNF-04); al più 60 eventi al minuto per IP su `POST /v1/events` e `/v1/transactions` (docs/11 §11; Q-P8: primo indirizzo di `X-Forwarded-For`, loopback esente, 429 `RATE_LIMITED` con `Retry-After`).
...
| TB-PLT-RLM-002 | 1 eventi da `203.0.113.10` | accettati=0 limitati=1 — divergenza corretta D-18 | Q-P8 | `TestbookPltApiIT#rateLimit` · `rate-limit.csv` |
| TB-PLT-RLM-003 | 1 eventi da `203.0.113.11` | accettati=1 limitati=0 | docs/11 §11; Q-P8 | `TestbookPltApiIT#rateLimit` · `rate-limit.csv` |
| TB-PLT-RLM-004 | 61 eventi da `203.0.113.12, 10.0.0.1` | accettati=60 limitati=1 — divergenza corretta D-18 | docs/11 §11; Q-P8 | `TestbookPltApiIT#rateLimit` · `rate-limit.csv` |
| TB-PLT-RLM-005 | 61 eventi da `loopback` | accettati=61 limitati=0 | Q-P8 | `TestbookPltApiIT#rateLimit` · `rate-limit.csv` |
...
| D-02 | ERR-018, ERR-019, HER-009, HER-010 | docs/06 §2: errori del client, mai 500 (Q-P2) | 405/415 resi come 500 `INTERNAL_ERROR` | `GlobalExceptionHandler.java:98` | corretta: errori `ErrorResponse` 4xx di Spring con il loro stato, famiglia `bad-request` |
| D-03 | ERR-007 | Q-P2: titoli in italiano | `title` «Gone» | `GlobalExceptionHandler.java:125` (`getReasonPhrase`) | corretta |
...
| D-05 | DLR-010…015, DLK-004, DLK-005, BUS-009, BRT-001…004 | docs/04 §5: deserializzazione non ritentabile | JSON illeggibile ritentato 3 volte; `lh-attempts` 3 per errori che Spring Kafka non ritenta | `DlqRecords.java:48-50` (`retryable`) | corretta (Q-P3) |
| D-06 | DLR-019…022, DLK-006 | Q-P3: la causa non ritentabile decide | errore avvolto ritentato col codice dell'involucro | `DlqRecords.java:37-50` | corretta |
...
| D-10 | CLK-011, CLK-012, CLK-059 | docs/10 §1.2 «combinabili» (Q-P5) | `@eom+1M` dal 30 settembre = 30 ottobre; testo estraneo tra scostamenti accettato | `SeedDates.java:98-116` | corretta |
| D-11 | VAR-001, 003…005, 008…016, 018, 020, 022, 024; HLR-003 | docs/11 §8 e §6 | variabili della matrice non lette; readiness senza db e kafka | nessun codice (**regola non implementata**) | corretta: `LhEnvironmentAliases` (Q-P9) |
...
| D-13 | REL-021 | docs/06 §4, RNF-07: `processed_event` > 14 giorni | nessuna pulizia | nessun codice (**regola non implementata**, docs/17 §5.12 n. 5) | corretta: `ProcessedEventCleanup` (Q-P4) |
| D-14 | PAG-004, 005, 010, 011, 015…017, 022, 023, 028, 029, 034, 035, 040, 041, 046, 047, 052, 053 | docs/06 §2 (Q-P1) | `size=0`/`page=-1` corretti in silenzio (200); membri di un segmento fino a 200 | `MemberService.java:125`, `SegmentService.java:71, 84`, `WebhookService.java:187`, `InboxService.java:113, 124`, `ContestController.java:156`, `RedemptionService.java:383`, `CouponService.java:195` | corretta: `PageParams` di lh-common; l'editor BO-04 carica l'elenco statico a pagine da 100 |
...
| D-17 | INF-001…003 | docs/06 §10, BO-30: `GET /v1/demo/info` | 404 | nessun codice (**regola non implementata**, docs/17 §5.12 n. 8) | corretta (Q-P7) |
| D-18 | RLM-002, RLM-004 | docs/11 §11: 60 eventi/min per IP | nessun limite | nessun codice (**regola non implementata**) | corretta: `IngressRateLimitFilter` (Q-P8) |
| D-19 | CTR-002…005, 007…011, 013, 014, 029 | docs/05 §9: uno schema e un esempio per ogni `type` | 12 schemi ed esempi assenti | `contracts/events/` | corretta: schemi ed esempi aggiunti (Q-P11) |
...
| D-22 | HLR-008 | docs/06 §2: OpenAPI su `/v3/api-docs` | 404 | nessuna dipendenza springdoc (**regola non implementata**) | **aperta**: serve una dipendenza nuova (Q-P10) |

## 26. Scelte registrate

docs/15 Q-P1…Q-P11 (tutte DECISA, opzione conservativa, tranne Q-P10 APERTA); Q-168 superata da Q-P7. Riepilogo:
Q-P1 paginazione 400 per `size` < 1 e `page` < 0 · Q-P2 errori di forma di Spring e titoli · Q-P3 classificazione
DLQ lungo la catena delle cause · Q-P4 `processed_event` 14 giorni · Q-P5 `@eom` con scostamenti e grammatica stretta ·
Q-P6 `@last<Giorno>` · Q-P7 reset serializzati e forma di `/v1/demo/info` · Q-P8 limite di frequenza · Q-P9 variabili
d'ambiente e sonde · Q-P10 OpenAPI · Q-P11 schemi aggiunti ai contratti. Valgono anche Q-131 (3 tentativi), Q-261 e
...
| `TestbookPltClockTest` | `SeedDates.applyOffsets`: `@eom` + mesi senza aggancio alla fine del mese (Q-P5) | CLK-011, CLK-012 | rilevata, ripristinata |
| `TestbookPltConfigTest` | `LhKafkaHealthIndicator`: cache della salute Kafka a 0 (docs/06 §8) | HLT-004 | rilevata, ripristinata |
| `TestbookPltDlqTest` | `DlqRecords.retryable`: elenco dei tipi non ritentabili ignorato (docs/04 §5, Q-P3) | DLR-010…015, DLR-022 | rilevata, ripristinata |
...
| `TestbookPltApiIT` | `PageParams.of`: `size` = 0 accettato (Q-P1) | PAG-004, 010, 016, 022, 028, 034, 040, 046, 052 (più HLR-008, già rossa: D-22) | rilevata, ripristinata |
...
| Rami senza specifica | 9 (innocui: mantenuti o decisi in Q-P2, Q-P6, Q-P7) |
| Regole senza codice | 8 prima delle correzioni (variabili d'ambiente, `lh_outbox_pending`, pulizia `processed_event`, `/v1/demo/info`, limite di frequenza, 12 contratti, profilo free lazy, OpenAPI); 1 dopo (OpenAPI, Q-P10); `member.birthday` P2 fuori perimetro |
...
| Divergenze | 22 cause, 108 righe: 21 corrette, 1 aperta (D-22, Q-P10) |
