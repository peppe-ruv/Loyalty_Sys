...
- **AMBIGUO** nella colonna *atteso*: la specifica tace e docs/15 non registra una scelta; il test asserisce il
  comportamento attuale con il commento `// TESTBOOK: ambiguo, vedi <ID>` (o la riga del CSV lo dichiara).
...
**Rami senza specifica** (mappati su righe AMBIGUO che asseriscono il comportamento attuale):
...
**Regole non implementate** (righe DIVERGENZA, rosse finché il codice manca): R-14 storico demo (MON-010), R-23
...
R-28 (pulizia di `inbound_event`) non ha codice né un punto d'ingresso da provare: nessuna riga eseguibile finché non esiste
il job.
...
| TB-ING-SRC-067 | fonte in forma breve senza URN (ecommerce) | AMBIGUO: ACCEPTED (comportamento attuale; docs/05 §2 prevede l'URN) | docs/05 §2 | `TestbookIngPipelineIT#src` |
...
| TB-ING-SRC-070 | URN di fonte con codice vuoto (urn:loyaltyhub:source:) | AMBIGUO: REJECTED/SOURCE_DISABLED (nessun 400) | docs/05 §2; envelope.schema.json | `TestbookIngPipelineIT#src` |
...
| TB-ING-TYP-010 | tipo custom senza JSON Schema, data qualunque | AMBIGUO: ACCEPTED, il passo 4 non respinge (ramo senza specifica) | ingestion §5.4; docs/17 US-E01-03 | `TestbookIngPipelineIT#typ` |
| TB-ING-TYP-011 | tipo custom da fonte con elenco che non lo contiene (ecommerce) | AMBIGUO: REJECTED/TYPE_NOT_ALLOWED (docs/05 §3: custom da «qualsiasi fonte») | ingestion §5.3; docs/05 §3 EVT-ACT-9x | `TestbookIngPipelineIT#typ` |
...
| TB-ING-SCH-083 | quiz.completed: correctAnswers 11 &gt; totalQuestions 10 · data `{"quizId":"QZ-1","correctAnswers":11,"totalQuestions":10}` | AMBIGUO: ACCEPTED | docs/05 §3 EVT-ACT-08; seed/event-types.json (nessun vincolo incrociato) | `TestbookIngPipelineIT#sch` |
...
(720 ore o 30 giorni di calendario a Roma): righe marcate AMBIGUO che asseriscono il comportamento attuale (720 ore).
...
| TB-ING-TIM-022 | cambio dell'ora di marzo: evento 30 gg prima alle 09:30 CET (dentro 720 h, fuori da 30 gg di calendario) · servizio `2026-04-15T08:00:00Z` · time `2026-03-16T08:30:00Z` | AMBIGUO: ACCEPTED (finestra di 720 ore, non di 30 giorni di calendario Europe/Rome) | ingestion §5.5 | `TestbookIngPipelineIT#tim` |
...
| TB-ING-TIM-024 | cambio dell'ora di ottobre: evento 30 gg di calendario prima alle 10:30 CEST (fuori da 720 h) · servizio `2026-11-10T09:00:00Z` · time `2026-10-11T08:30:00Z` | AMBIGUO: REJECTED/INVALID_TIME (720 ore; 30 gg di calendario lo ammetterebbero) | ingestion §5.5 | `TestbookIngPipelineIT#tim` |
...
conti: righe AMBIGUO che asseriscono il comportamento attuale (non conta), coerente con la riprova (F-ING-09).
...
| TB-ING-DUP-006 | primo invio REJECTED/INVALID_DATA, secondo corretto con lo stesso id | AMBIGUO: secondo ACCEPTED (conta solo un ACCEPTED; F-ING-02 dice «già visto») | F-ING-02; ingestion §5.6; docs/17 US-E01-04 | `TestbookIngPipelineIT#dup` |
| TB-ING-DUP-007 | primo invio UNMATCHED, poi il membro arriva e la fonte rimanda lo stesso id | AMBIGUO: secondo ACCEPTED | F-ING-02; ingestion §5.6; docs/17 US-E01-04 | `TestbookIngPipelineIT#dup` |
| TB-ING-DUP-008 | due invii per un membro BLOCKED | AMBIGUO: due righe REJECTED/MEMBER_NOT_ACTIVE, nessun DUPLICATE | F-ING-02; ingestion §5.6 | `TestbookIngPipelineIT#dup` |
...
prevista: righe AMBIGUO col comportamento attuale (trattato come id).
...
| TB-ING-MBR-025 | &lt;id&gt; senza prefisso · membro assente dall'indice | AMBIGUO: UNMATCHED (subject senza prefisso trattato come id) | F-ING-03 (forme ammesse: member:/external:/email:); docs/17 US-E01-05 | `TestbookIngPipelineIT#mbr` |
| TB-ING-MBR-026 | &lt;id&gt; senza prefisso · membro ACTIVE | AMBIGUO: ACCEPTED + memberId (subject senza prefisso trattato come id) | F-ING-03 (forme ammesse: member:/external:/email:); docs/17 US-E01-05 | `TestbookIngPipelineIT#mbr` |
| TB-ING-MBR-027 | &lt;id&gt; senza prefisso · membro INACTIVE | AMBIGUO: REJECTED/MEMBER_NOT_ACTIVE + memberId (subject senza prefisso trattato come id) | F-ING-03 (forme ammesse: member:/external:/email:); docs/17 US-E01-05 | `TestbookIngPipelineIT#mbr` |
| TB-ING-MBR-028 | &lt;id&gt; senza prefisso · membro BLOCKED | AMBIGUO: REJECTED/MEMBER_NOT_ACTIVE + memberId (subject senza prefisso trattato come id) | F-ING-03 (forme ammesse: member:/external:/email:); docs/17 US-E01-05 | `TestbookIngPipelineIT#mbr` |
| TB-ING-MBR-029 | &lt;id&gt; senza prefisso · membro anonimizzato (cancellazione reale) | AMBIGUO: REJECTED/MEMBER_NOT_ACTIVE + memberId (subject senza prefisso trattato come id) | F-ING-03 (forme ammesse: member:/external:/email:); docs/17 US-E01-05 | `TestbookIngPipelineIT#mbr` |
| TB-ING-MBR-030 | &lt;id&gt; senza prefisso · stato sconosciuto SUSPENDED | AMBIGUO: REJECTED/MEMBER_NOT_ACTIVE + memberId (subject senza prefisso trattato come id) | F-ING-03 (forme ammesse: member:/external:/email:); docs/17 US-E01-05 | `TestbookIngPipelineIT#mbr` |
| TB-ING-MBR-031 | external: con maiuscole diverse dall'indice | AMBIGUO: UNMATCHED (confronto esatto sull'externalId) | F-ING-03 | `TestbookIngPipelineIT#mbr` |
| TB-ING-MBR-032 | prefisso sconosciuto phone: | AMBIGUO: UNMATCHED (forma non prevista) | F-ING-03 | `TestbookIngPipelineIT#mbr` |
| TB-ING-MBR-033 | member: senza id | AMBIGUO: UNMATCHED | F-ING-03; ingestion §5.7 | `TestbookIngPipelineIT#mbr` |
...
| TB-ING-ACC-006 | subject sulla riga di un ingresso accettato | AMBIGUO: la riga ACCEPTED porta il subject normalizzato member:&lt;id&gt; (per riprova/abbina Q-118 conserva invece l'originale) | ingestion §2; Q-118 | `TestbookIngPipelineIT#acc` |
...
| TB-ING-RES-007 | Riprova di un UNMATCHED risolvibile · intestazione non valida PIRATA:x | AMBIGUO: 403 FORBIDDEN_ROLE, riga UNMATCHED, nulla pubblicato (ruolo non valido trattato come ANALYST) | ingestion §3; docs/08 §2 (inbound.handle: ADMIN, CARE); docs/06 §3 | `TestbookIngResolutionIT#guard` |
...
| TB-ING-RES-014 | Abbina di un UNMATCHED risolvibile · intestazione non valida PIRATA:x | AMBIGUO: 403 FORBIDDEN_ROLE, riga UNMATCHED, nulla pubblicato (ruolo non valido trattato come ANALYST) | ingestion §3; docs/08 §2 (inbound.handle: ADMIN, CARE); docs/06 §3 | `TestbookIngResolutionIT#guard` |
...
| TB-ING-RES-050 | Abbina: membro non indicizzato | AMBIGUO: 422 MEMBER_NOT_FOUND (codice non specificato) | ingestion §3 (abbina {memberId} un UNMATCHED); F-ING-04 | `TestbookIngResolutionIT#matchOutcome` |
| TB-ING-RES-051 | Abbina: memberId di soli spazi | AMBIGUO: 422 MEMBER_REQUIRED | ingestion §3 (abbina {memberId} un UNMATCHED); F-ING-04 | `TestbookIngResolutionIT#matchOutcome` |
| TB-ING-RES-052 | Abbina: corpo assente | AMBIGUO: 422 MEMBER_REQUIRED | ingestion §3 (abbina {memberId} un UNMATCHED); F-ING-04 | `TestbookIngResolutionIT#matchOutcome` |
| TB-ING-RES-053 | Abbina: corpo {} senza memberId | AMBIGUO: 422 MEMBER_REQUIRED | ingestion §3 (abbina {memberId} un UNMATCHED); F-ING-04 | `TestbookIngResolutionIT#matchOutcome` |
| TB-ING-RES-054 | Abbina: memberId con spazi attorno | AMBIGUO: 200 ACCEPTED (spazi ignorati) | ingestion §3 (abbina {memberId} un UNMATCHED); F-ING-04 | `TestbookIngResolutionIT#matchOutcome` |
...
| TB-ING-ETY-009 | codice di 60 caratteri | AMBIGUO: 201 (limite di 60 non specificato) | Q-89 (codice minuscolo a punti, 2–4 parti; categorie TRANSACTION/ENGAGEMENT/SERVICE) | `TestbookIngConfigIT#ety` |
| TB-ING-ETY-010 | codice di 61 caratteri | AMBIGUO: 422 (limite di 60 non specificato) | Q-89 (codice minuscolo a punti, 2–4 parti; categorie TRANSACTION/ENGAGEMENT/SERVICE) | `TestbookIngConfigIT#ety` |
...
| TB-ING-ETY-015 | nome di 60 caratteri | AMBIGUO: 201 (limite non specificato) | ingestion §3 (event-types); F-ING-06; Q-89 | `TestbookIngConfigIT#ety` |
| TB-ING-ETY-016 | nome di 61 caratteri | AMBIGUO: 422 (limite non specificato) | ingestion §3 (event-types); F-ING-06; Q-89 | `TestbookIngConfigIT#ety` |
...
| TB-ING-ETY-022 | categoria assente | AMBIGUO: 201 con categoria ENGAGEMENT | Q-89 (codice minuscolo a punti, 2–4 parti; categorie TRANSACTION/ENGAGEMENT/SERVICE) | `TestbookIngConfigIT#ety` |
...
| TB-ING-ETY-027 | sampleData assente con schema senza obbligatori | AMBIGUO: 201 con sampleData {} | ingestion §3 (event-types); F-ING-06; Q-89 | `TestbookIngConfigIT#ety` |
...
| TB-ING-ETY-029 | abilitazione assente ⇒ abilitato | AMBIGUO: 201 enabled=true | ingestion §3 (event-types); F-ING-06; Q-89 | `TestbookIngConfigIT#ety` |
...
| TB-ING-ETY-034 | crea con X-LH-Actor non valido | AMBIGUO: 403 (ruolo sconosciuto = ANALYST) | docs/06 §3 | `TestbookIngConfigIT#ety` |
...
| TB-ING-ETY-057 | ADMIN manda un nome vuoto a un tipo di sistema | AMBIGUO: 200, il nome resta quello di prima | ingestion §3 | `TestbookIngConfigIT#ety` |
...
| TB-ING-TXN-003 | kind purchase minuscolo | AMBIGUO: accettato (maiuscole ignorate) | Q-49 | `TestbookIngConfigIT#txn` |
...
| TB-ING-TXN-007 | acquisto senza currency | AMBIGUO: 400 (errore di forma, non INVALID_DATA) | ingestion §3 (POST /v1/transactions); F-ING-07; docs/17 US-E01-08 | `TestbookIngConfigIT#txn` |
...
| TB-ING-TXN-012 | senza amount | AMBIGUO: 400 (errore di forma, non INVALID_DATA) | ingestion §3 (POST /v1/transactions); F-ING-07 | `TestbookIngConfigIT#txn` |
...
| TB-ING-SIM-011 | count 21 | AMBIGUO: 20 eventi (limitato, non rifiutato) | BO-28 | `TestbookIngConfigIT#sim` |
| TB-ING-SIM-012 | count 0 | AMBIGUO: 1 evento (limitato, non rifiutato) | BO-28 | `TestbookIngConfigIT#sim` |
| TB-ING-SIM-013 | count -3 | AMBIGUO: 1 evento | BO-28 | `TestbookIngConfigIT#sim` |
...
| TB-ING-SCN-023 | expect non rispettato | passo ok=false; esecuzione DONE | AMBIGUO: ingestion §5 (esito dell'esecuzione non specificato) | `TestbookIngConfigIT#scn` |
...
| TB-ING-SCN-025 | at non valido ("domani") | AMBIGUO: esecuzione FAILED (esito di un passo non valido non specificato) | docs/10 §1; ingestion §2 (scenario_run.status) | `TestbookIngConfigIT#scn` |
| TB-ING-SCN-026 | passo senza source | AMBIGUO: fonte simulator | docs/10 §8 | `TestbookIngConfigIT#scn` |
...
| TB-ING-SCT-008 | @lastWeekdayT10:30 una domenica · at `@lastWeekdayT10:30` · adesso `2026-09-27T12:00` (Roma) | AMBIGUO (feriale = lun–ven): → 2026-09-25T10:30 (Roma) | docs/10 §1 (espressioni di data, Europe/Rome); docs/10 §8 | `TestbookIngScenarioTimeTest#sct` |
...
| TB-ING-SCT-016 | @lastSundayT02:30 nell'ora che non esiste (29/3/2026) · at `@lastSundayT02:30` · adesso `2026-03-30T12:00` (Roma) | AMBIGUO (ora inesistente ⇒ 03:30 CEST): → 2026-03-29T01:30:00Z UTC | docs/10 §1 (espressioni di data, Europe/Rome); docs/10 §8 | `TestbookIngScenarioTimeTest#sct` |
...
| TB-ING-SCT-018 | @lastSundayT02:30 nell'ora ripetuta (25/10/2026) · at `@lastSundayT02:30` · adesso `2026-10-26T12:00` (Roma) | AMBIGUO (ora doppia ⇒ prima occorrenza, CEST): → 2026-10-25T00:30:00Z UTC | docs/10 §1 (espressioni di data, Europe/Rome); docs/10 §8 | `TestbookIngScenarioTimeTest#sct` |
...
| Rami senza specifica | 17 (§2), 47 righe AMBIGUO |
| Regole non implementate | 6 (R-14, R-23, filtri di R-13, parti di R-25 e R-27, R-28) |
| Divergenze | 46 righe, 14 cause (§5) |

## 5. Registro delle divergenze

| N. | Righe | Specifica | Comportamento osservato | Causa (file:riga) |
|---|---|---|---|---|
| 1 | SCH-011, 018, 020, 030, 101, 115, 117, 118, 121, 122, 123, 127, 128, 131, 134, 139, 140, 144, 148; TXN-019 | `contracts/events/action/*.schema.json` (precedenza 2): `amount` ≥ 0, `currency` minLength 1, `quantity` ≥ 0, pattern dei codici, `points`/`pointsCost` ≥ 1, `counterpartMemberId` obbligatorio, enum `origin`, minLength 1 | la validazione usa gli schemi di `seed/event-types.json`, più stretti (`exclusiveMinimum 0`, `currency` 3 caratteri, `quantity` ≥ 1) o più larghi (nessun pattern/enum/required, minimum 0) dei contratti | `seed/event-types.json` (`dataSchema` dei tipi); `IngestionService.java:153-158` |
| 2 | FRM-024, FRM-025 | envelope: `data` è un oggetto; errori di forma ⇒ 400 (ingestion §5.1; `envelope.schema.json`) | `202 REJECTED/INVALID_DATA` e riga salvata | `IngestionService.java:237` (controlla solo `null`) |
...
| 4 | SRC-068, SRC-069 | `source` di un'azione = `urn:loyaltyhub:source:<codice>` (docs/05 §2) | qualunque stringa che termina con `:ecommerce` vale come fonte `ecommerce` ⇒ ACCEPTED | `IngestionService.java:293-300` (`normalizeSource` + ultimo segmento dopo `:`) |
| 5 | MON-008, MON-009 | filtri `from`, `to`, `q` (ingestion §3) | parametri ignorati | `InboundEventsController.java:55-61` |
| 6 | MON-010 | storico di 40 `inbound_event` degli ultimi 3 giorni (ingestion §6) | nessuna riga seminata | `DemoSeeder` (nessun seed di `inbound_event`) |
| 7 | FON-002, FON-003, FON-004 | `POST`/`PUT /v1/sources` (ingestion §3; F-ING-05) | `PUT` 404, `POST` 500 | `RegistryController.java:22` (solo `GET`) |
| 8 | ETY-007 | codice custom minuscolo (Q-89) | `x.readingSent` accettato | `EventTypeService.java:42` (`[a-zA-Z0-9]` dopo il primo carattere) |
| 9 | ETY-025 | lo schema di un custom è un JSON Schema valido (F-ING-06) | `{"type":"nonsense"}` accettato | `EventTypeService.java:146-158` (la compilazione non verifica il meta-schema) |
| 10 | SIM-008 | origine `SIMULATOR` per gli eventi del simulatore (ingestion §2) | origine `EXTERNAL` | `SimulatorController.java:62` (`ingest(event)` senza origine) |
| 11 | SIM-015 | `data` assente ⇒ `sample_data` con piccole variazioni casuali (ingestion §3) | `sample_data` identico | `SimulatorController.java:55`, `#sampleData` |
| 12 | SCN-024, SCT-022, SCT-023 | `at` è un'espressione di docs/10 §1 (default `@now`) | `@now`, `@today…` non riconosciuti (esecuzione `FAILED`) | `ScenarioTime.java:28-30` (`Instant.parse`) |
| 13 | SCT-003, SCT-006, SCT-010, SCT-019 | `@lastWeekday`/`@lastSaturday` = giorno **precedente a oggi** (docs/10 §1) | se l'orario di oggi è già passato si usa oggi | `ScenarioTime.java:38-41` (`back = 0` ammesso) |
| 14 | SCT-021 | `@lastWeekday` senza orario è valido (docs/10 §1) | eccezione «senza orario» | `ScenarioTime.java:32-33` |

## 6. Ambiguità (righe AMBIGUO)

TB-ING-ACC-006, TB-ING-DUP-006, TB-ING-DUP-007, TB-ING-DUP-008, TB-ING-ETY-009, TB-ING-ETY-010, TB-ING-ETY-015, TB-ING-ETY-016, TB-ING-ETY-022, TB-ING-ETY-027, TB-ING-ETY-029, TB-ING-ETY-034, TB-ING-ETY-057, TB-ING-MBR-025, TB-ING-MBR-026, TB-ING-MBR-027, TB-ING-MBR-028, TB-ING-MBR-029, TB-ING-MBR-030, TB-ING-MBR-031, TB-ING-MBR-032, TB-ING-MBR-033, TB-ING-RES-007, TB-ING-RES-014, TB-ING-RES-050, TB-ING-RES-051, TB-ING-RES-052, TB-ING-RES-053, TB-ING-RES-054, TB-ING-SCH-083, TB-ING-SCN-025, TB-ING-SCN-026, TB-ING-SCT-008, TB-ING-SCT-016, TB-ING-SCT-018, TB-ING-SIM-011, TB-ING-SIM-012, TB-ING-SIM-013, TB-ING-SRC-067, TB-ING-SRC-070, TB-ING-TIM-022, TB-ING-TIM-024, TB-ING-TXN-003, TB-ING-TXN-007, TB-ING-TXN-012, TB-ING-TYP-010, TB-ING-TYP-011.

Le principali, da registrare in docs/15: subject senza prefisso (F-ING-03); dedup dopo un `REJECTED`/`UNMATCHED` (F-ING-02
«già visto»); «30 giorni» al cambio dell'ora (720 ore o giorni di calendario); `source` in forma breve; codici e limiti
dell'Abbina e dei tipi custom; tipi custom da fonti con elenco (docs/05 §3 «qualsiasi fonte»); `count` fuori 1–20;
esito di uno scenario con `expect` non rispettato o `at` non valido; ora inesistente/doppia nelle espressioni `at`.
