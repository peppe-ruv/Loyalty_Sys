...
| ⚠ `ContestAdminService.validate` :418, :426 | meccanica o tipo premio `null` → `List.of(..).contains(null)` lancia NPE → 500 | R21 | EDT-017, PRZ-013 |
...
| ⚠ `ContestAdminService.update` :149–158 | `LIVE/PAUSED`: premi, periodo (anche `endAt`), distribuzione, seme, meccanica, gratuita, limiti → 409 `CONTEST_LIVE_LOCKED`; regolamento ammesso | R22 | EDT-050…063 |
...
| ⚠ `AchievementRules.matches/field/rule` :61–150 | solo `all`/`any` piatti; comparatori `eq neq gt gte lt lte in`; campo assente → falso (corretto su main); niente `[*]` | R35 | FLT |
...
| ⚠ `AchievementAdminService.build` :113–114 | metrica `null` → NPE → 500; periodi ammessi solo `EVER, MONTH, EDITION` (Q-159) | R39, R34 | ACF-002, ACF-017…019 |
...
| ⚠ `LeaderboardService.build` :131 | metrica `null` → NPE → 500 | R43 | LCF-024 |
...
| TB-GAM-PRZ-013 | tipo assente (tipo -, punti -, rewardCode -, quantità 3) | 422 `CONTEST_INVALID`, nessun concorso creato **DIVERGENZA** | F-IW-02, BO-14 (tipo: POINTS n / COUPON premio / PHYSICAL), docs/06 §2 (422), contratto `fact.contest.won` (`points` ≥ 1) | `TestbookGamPrizeIT#validazione` |
...
| TB-GAM-EDT-017 | meccanica assente (`mechanic` = assente); ruolo MARKETING | 422 **DIVERGENZA** | F-IW-01, gamification §2, Q-56 | `TestbookGamContestIT#creazione` |
...
| TB-GAM-EDT-052 | LIVE: proroga di endAt (campo sicuro per docs/03 §3.6) (`PUT` del solo `endAt`) | 200, campo aggiornato; istanti invariati **DIVERGENZA** | docs/03 §3.6 (campi sicuri: nome, descrizione, `endAt`), §7 (istanti immutabili dopo l'avvio), gamification §3 (premi solo prima di LIVE), docs/06 §2 (409) | `TestbookGamContestIT#modifica` |
| TB-GAM-EDT-053 | LIVE: cambio del regolamento (non tra i campi sicuri) (`PUT` del solo `rulesText`) | 409 `CONTEST_LIVE_LOCKED`, concorso invariato; istanti invariati **DIVERGENZA** | docs/03 §3.6 (campi sicuri: nome, descrizione, `endAt`), §7 (istanti immutabili dopo l'avvio), gamification §3 (premi solo prima di LIVE), docs/06 §2 (409) | `TestbookGamContestIT#modifica` |
...
| TB-GAM-FLT-019 | nin con valore fuori elenco: filtro `{"op":"all","rules":[{"field":"data.channel","cmp":"nin","value":["APP"]}]}` su `{"channel":"POS"}` | vero (conta) **DIVERGENZA** | docs/03 §8 (filtro = condizioni su `data.*`), §3.3 (comparatori, gruppi, campo assente, tipi incompatibili) | `TestbookGamAchievementRulesTest#filtro` |
| TB-GAM-FLT-020 | contains su un elenco che contiene il valore: filtro `{"op":"all","rules":[{"field":"data.tags","cmp":"contains","value":"a"}]}` su `{"tags":["a","b"]}` | vero (conta) **DIVERGENZA** | docs/03 §8 (filtro = condizioni su `data.*`), §3.3 (comparatori, gruppi, campo assente, tipi incompatibili) | `TestbookGamAchievementRulesTest#filtro` |
| TB-GAM-FLT-021 | ncontains su un testo che non contiene il valore: filtro `{"op":"all","rules":[{"field":"data.code","cmp":"ncontains","value":"z"}]}` su `{"code":"abc"}` | vero (conta) **DIVERGENZA** | docs/03 §8 (filtro = condizioni su `data.*`), §3.3 (comparatori, gruppi, campo assente, tipi incompatibili) | `TestbookGamAchievementRulesTest#filtro` |
| TB-GAM-FLT-022 | exists su campo presente: filtro `{"op":"all","rules":[{"field":"data.amount","cmp":"exists"}]}` su `{"amount":50}` | vero (conta) **DIVERGENZA** | docs/03 §8 (filtro = condizioni su `data.*`), §3.3 (comparatori, gruppi, campo assente, tipi incompatibili) | `TestbookGamAchievementRulesTest#filtro` |
...
| TB-GAM-FLT-024 | nexists su campo assente: filtro `{"op":"all","rules":[{"field":"data.amount","cmp":"nexists"}]}` su `{"channel":"APP"}` | vero (conta) **DIVERGENZA** | docs/03 §8 (filtro = condizioni su `data.*`), §3.3 (comparatori, gruppi, campo assente, tipi incompatibili) | `TestbookGamAchievementRulesTest#filtro` |
...
| TB-GAM-FLT-026 | between 10 e 100 con 50: filtro `{"op":"all","rules":[{"field":"data.amount","cmp":"between","value":[10,100]}]}` su `{"amount":50}` | vero (conta) **DIVERGENZA** | docs/03 §8 (filtro = condizioni su `data.*`), §3.3 (comparatori, gruppi, campo assente, tipi incompatibili) | `TestbookGamAchievementRulesTest#filtro` |
| TB-GAM-FLT-027 | startsWith AP su APP: filtro `{"op":"all","rules":[{"field":"data.channel","cmp":"startsWith","value":"AP"}]}` su `{"channel":"APP"}` | vero (conta) **DIVERGENZA** | docs/03 §8 (filtro = condizioni su `data.*`), §3.3 (comparatori, gruppi, campo assente, tipi incompatibili) | `TestbookGamAchievementRulesTest#filtro` |
...
| TB-GAM-FLT-035 | not su una regola falsa: filtro `{"op":"not","rules":[{"field":"data.amount","cmp":"gte","value":100}]}` su `{"amount":50}` | vero (conta) **DIVERGENZA** | docs/03 §8 (filtro = condizioni su `data.*`), §3.3 (comparatori, gruppi, campo assente, tipi incompatibili) | `TestbookGamAchievementRulesTest#filtro` |
| TB-GAM-FLT-036 | campo su elenco items[*].category (almeno un elemento): filtro `{"op":"all","rules":[{"field":"data.items[*].category","cmp":"eq","value":"food"}]}` su `{"items":[{"category":"tech"},{"category":"food"}]}` | vero (conta) **DIVERGENZA** | docs/03 §8 (filtro = condizioni su `data.*`), §3.3 (comparatori, gruppi, campo assente, tipi incompatibili) | `TestbookGamAchievementRulesTest#filtro` |
...
| TB-GAM-FLT-038 | gruppo annidato any dentro all: filtro `{"op":"all","rules":[{"field":"data.amount","cmp":"gte","value":10},{"op":"any","rules":[{"field":"data.channel","cmp":"eq","value":"APP"},{"field":"data.channel","cmp":"eq","value":"WEB"}]}]}` su `{"amount":20,"channel":"WEB"}` | vero (conta) **DIVERGENZA** | docs/03 §8 (filtro = condizioni su `data.*`), §3.3 (comparatori, gruppi, campo assente, tipi incompatibili) | `TestbookGamAchievementRulesTest#filtro` |
...
| TB-GAM-ACF-002 | metrica assente (`metric=~`; ruolo MARKETING) | 422 `ACHIEVEMENT_INVALID` **DIVERGENZA** | F-ACH-01, BO-15, gamification §2, docs/06 §2 | `TestbookGamAchievementIT#configurazione` |
...
| TB-GAM-LCF-024 | metrica assente (`metric=~`; ruolo MARKETING) | 422 `LEADERBOARD_INVALID` **DIVERGENZA** | F-LDB-01, BO-16, gamification §2 (metriche e periodi), docs/06 §2 | `TestbookGamLeaderboardIT#configurazione` |
...
| Rami del codice mappati (§1.2) | 78 nodi, di cui 19 con almeno un *ramo senza specifica* e 5 in contrasto con la specifica (⚠) |
| Righe del testbook | 710 |
| Righe **AMBIGUO** (comportamento attuale, domanda da registrare) | 56 |
| Righe in **DIVERGENZA** (test rossi finché il codice non segue la specifica) | 16 |
...
- **R35** — grammatica delle condizioni di docs/03 §3.3 nel filtro degli obiettivi: mancano `nin, contains, ncontains, exists, nexists, between, startsWith`, il gruppo `not`, i gruppi annidati e i campi su elenco `[*]`. Righe TB-GAM-FLT-019…022, 024, 026, 027, 035, 036, 038 (rosse). Il campo assente con `neq` (TB-GAM-FLT-029) è stato corretto su main (6b1b964) ed è verde.
...
| # | Riga | Specifica | Comportamento osservato | Causa (file:riga) |
|---|---|---|---|---|
| 1 | TB-GAM-EDT-017 | docs/06 §2: regola violata → 422 `CONTEST_INVALID` (meccanica obbligatoria, gamification §2) | 500 `INTERNAL_ERROR` con `mechanic` assente | `ContestAdminService.java:418` — `Contest.MECHANICS` è `List.of(…)`: `contains(null)` lancia `NullPointerException` |
| 2 | TB-GAM-PRZ-013 | docs/06 §2, gamification §2 (`type` del premio) → 422 | 500 con `type` del premio assente | `ContestAdminService.java:426` — `Prize.TYPES.contains(null)` |
| 3 | TB-GAM-ACF-002 | docs/06 §2, F-ACH-01 (metrica obbligatoria) → 422 `ACHIEVEMENT_INVALID` | 500 con `metric` assente | `AchievementAdminService.java:113` — `Achievement.METRICS.contains(null)` |
| 4 | TB-GAM-LCF-024 | docs/06 §2, F-LDB-01 → 422 `LEADERBOARD_INVALID` | 500 con `metric` assente | `LeaderboardService.java:131` — `Leaderboard.METRICS.contains(null)` |
| 5 | TB-GAM-EDT-052 | docs/03 §3.6: un oggetto `LIVE` si modifica nei campi sicuri «nome, descrizione, `endAt`, priorità, immagine» | 409 `CONTEST_LIVE_LOCKED` alla proroga di `endAt` | `ContestAdminService.java:149–151, 155–158` — `endAt` conta tra i campi che toccano gli istanti e blocca il LIVE |
| 6 | TB-GAM-EDT-053 | docs/03 §3.6: fuori dai campi sicuri serve duplicare; il regolamento non è tra i campi sicuri | 200, regolamento cambiato su un concorso `LIVE` | `ContestAdminService.java:152–155` — `rulesText` escluso dal controllo (javadoc :125–127 «solo nome, descrizione e regolamento») |
...
Domande da registrare (nessuna modifica a docs/15 in questo compito):
1. Ordine dei controlli della giocata e codice quando più condizioni sono false (PLY-053…055, CRD con tetto raggiunto e nessuna giocata).
2. `memberId` assente o vuoto nella giocata e id al posto del codice nel path del portale (PLY-056, PLY-057, PLY-059).
3. Portale: concorsi `LIVE` fuori periodo, membro non attivo (PTL-002, PTL-003, PTL-006).
4. Istante piantato: stato del concorso, premio sconosciuto o assente, nessun istante aperto (PLT-006…009, PLT-017).
5. Concorsi `PAUSED` come `LIVE` per le modifiche; `ENDED/ARCHIVED` non modificabili; codice immutabile (EDT-040, EDT-044, EDT-045, EDT-062, EDT-063 — Q-51 vale solo per le campagne); codice minuscolo normalizzato, distribuzione di default (EDT-008, EDT-018).
6. Generazione senza premi o senza ore utili in `BUSINESS_HOURS` (INS-017, GEN-033); formato di `asOf` (END-008, come Q-156 per il wallet).
7. `SUM`: troncamento dei decimali, importi negativi o testuali, forma `tipo.data.campo` (MET-002…008, MET-013, PRG-018, PRG-021); serie con azione fuori ordine (STK-019); progresso di un membro senza snapshot (PRG-013); comparatore sconosciuto (FLT-037).
8. Configurazione: prefissi `ACH-`/`BDG-`/`LDB-`, `DISTINCT_TYPES` oltre i tipi, top N 3…50, metrica/periodo/codice immutabili (ACF-012, ACF-025, ACF-033, BDG-013, LCF-001, LCF-004, LCF-015, LCF-020, LCF-021, LCF-023).
9. Effetti: `count` assente o < 1, crediti verso un concorso non `LIVE`, codice DLQ per effetti senza dati o senza membro (GRT-004, GRT-005, GRT-007…009, BDG-005, BDG-006); nota di consegna vuota (PRZ-034); nickname senza nome (NCK-005).
...
- Rapporto (`scripts/testbook.sh`, `TESTBOOK_WEB=0`): TB-GAM — 710 righe, 694 OK, 16 fallite (le divergenze di §21.5), 0 non eseguite, 0 test senza riga.
