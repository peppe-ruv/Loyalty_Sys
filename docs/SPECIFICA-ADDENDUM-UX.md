...
Da inserire nella sezione «Backoffice e CMS» della `SPECIFICA-LOYALTY-HUB.md`, dopo i requisiti trasversali RF-40..RF-46. Riferimenti: ADR-026, D26, `docs/LINEE-GUIDA-UX-BACKOFFICE.md`.

## Linee guida UX (LG-01..LG-48)

Dal 17 settembre 2026 il backoffice segue le 48 linee guida derivate dalla playlist «Open Loyalty Feature Showcase»: navigazione a gruppi (Amministrazione, Generale, Moduli loyalty, Concorsi e programma, Contenuti, Decisioni), tabella standard con filtri a chip per ogni lista, form a sezioni fisse, condizioni scritte come frasi, effetti con formula guidata, profilo membro a due colonne con timeline, KPI come tab di un solo grafico con confronto al periodo precedente. Quindici pattern UI formano il design system (`web/backoffice-design-system/`), da costruire prima dei moduli.

**Requisiti UX (RF-137 – RF-142)**

- RF-137 Ogni oggetto configurabile espone lo stato di workflow di D14 (Bozza, In revisione, Approvato, Verifica Legal, Programmato, Pubblicato, Bloccato) come ultima sezione del form, con testo che spiega l'effetto dello stato e transizioni filtrate per ruolo; non esiste un toggle Attivo/Inattivo separato dal workflow.
...
- [ ] Marketing: estendere la galleria modelli (LG-31) alle campagne Iren tipiche (autolettura, bolletta digitale, domiciliazione)
...
- `web/backoffice-design-system/`: contratti dei 15 pattern (`patterns.ts`), token sulle fondamenta Design Tokens Italia (`tokens.css`, ADR-027), README con ordine di costruzione
