...
| 2026-09-23 | M3.4 | ✅ Implementato backend M3.4 (edizioni, chiusura soft descent e policy valute) | | — | **wallet-service** completato M3.4 [F-TIER-04][F-TIER-05] (solo backend): `EditionCloseRule` per soft descent, API `/v1/editions/{code}/close` con `dryRun`, elaborazione a lotti (in `EditionCloseBatchService`). Aggiunta gestione API valute e rispettive policy di scadenza (`CurrenciesController`). Frontend BO-08 ancora da sviluppare. |
