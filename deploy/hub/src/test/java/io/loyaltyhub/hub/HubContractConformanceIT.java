...
public class HubContractConformanceIT {
    ...
    private void awaitRunDone(String runId) {
        ...
        while (System.currentTimeMillis() < deadline) {
            JsonNode run = get("/v1/demo/scenario-runs/" + runId, null);
    ...
    void validateRuntimeEventContracts() throws Exception {
        ...
        // 7. Inbound retry/match
        post("/v1/events", null, Map.of(
                "specversion", "1.0", "id", "evt-unmatched-1", "source", "urn:loyaltyhub:source:ecommerce",
                "type", "purchase.completed", "subject", "member:MBR-UNKNOWN", "time", Instant.now().toString(),
                "data", Map.of("orderId", "ORD-UNKNOWN", "amount", 100, "currency", "EUR", "channel", "ONLINE")
        ));
        sleep(2000); // Give it time to process
        JsonNode inbounds = get("/v1/inbound-events?memberId=MBR-UNKNOWN", "ADMIN:test");
        if (inbounds.path("items").isArray() && !inbounds.path("items").isEmpty()) {
            String rowId = inbounds.path("items").get(0).path("id").asString();
        ...
        // Wait a bit for everything to sink into insight.event_store
        sleep(5000);
