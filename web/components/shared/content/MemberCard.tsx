...
// Tessera membro (docs/07 §5.3, docs/09 §3): materiale del tier, nome, numero, saldo punti in evidenza. Condivisa col
// backoffice (anteprima di BO-20): nome del programma e della valuta vengono dal tema (M6.5).
...
export function MemberCard({
  ...
  const { programName, currencyNames } = usePortalTheme();
  return (
    <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${style.bg} p-5 text-white shadow-lg`}>
      ...
      <div className="mt-6">
        <p className="text-xs uppercase tracking-wide opacity-80">Saldo {currencyNames.PTS}</p>
